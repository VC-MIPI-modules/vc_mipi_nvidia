/*
 * imx327c.c - imx327c sensor driver
...
#define VC_CODE             1   /* CCC - imx327c.c - enable code for VC MIPI camera  */
#define IMX327C_ENB_MUTEX   0   /* CCC - imx327c.c - mutex lock/unlock */
...
//#include <media/imx327c.h>

#include "../platform/tegra/camera/camera_gpio.h"
#include "vc_mipi.h"
//#include "imx327c.h"
//#include "imx327c_mode_tbls.h"
...
#define IMX327C_TABLE_WAIT_MS    0
#define IMX327C_TABLE_END        1




/* In dB*10 */
#define IMX327C_DIGITAL_GAIN_MIN        0
#define IMX327C_DIGITAL_GAIN_MAX        240
#define IMX327C_DIGITAL_GAIN_DEFAULT    0

/* In usec */
#define IMX327C_DIGITAL_EXPOSURE_MIN     29          // microseconds (us)
#define IMX327C_DIGITAL_EXPOSURE_MAX     7767184
#define IMX327C_DIGITAL_EXPOSURE_DEFAULT 10000

/* In dB*10 */
//#define IMX327C_ANALOG_GAIN_MIN          0
//#define IMX327C_ANALOG_GAIN_MAX          978
//#define IMX327C_ANALOG_GAIN_DEFAULT      0

// PK: just for test, not supported yet
//#define IMX327C_MIN_FRAME_LENGTH     100
//#define IMX327C_MAX_FRAME_LENGTH     800000
#define IMX327C_FRAME_RATE_DEFAULT   30000000   // 30 fps (*1000000)
...
#define imx327c_reg struct reg_8

static const imx327c_reg imx327c_start[] = {
    {0x3000, 0x00},     /* mode select streaming on */
    {0x3002, 0x00},     /* mode select streaming on */
    {IMX327C_TABLE_END, 0x00}
};

static const imx327c_reg imx327c_stop[] = {
...
static const imx327c_reg imx327c_mode_1920X1080[] = {
#if 1   // [[[
    { 0x3000, 0x1 },            // STANDBY[0]  = 0x3000 : Standby register: 0=Operating, 1=Standby
    { IMX327C_TABLE_WAIT_MS, 5 },
...
//static const imx327c_reg imx297_mode_720X540[] = {
...
static const imx327c_reg *imx327c_mode_table[] = {
    [IMX327C_MODE_1920X1080]    = imx327c_mode_1920X1080,
    [IMX327C_MODE_START_STREAM] = imx327c_start,
    [IMX327C_MODE_STOP_STREAM]  = imx327c_stop,
};

static const int imx327c_60fps[] = {
    60,
};

static const struct camera_common_frmfmt imx327c_frmfmt[] = {
    { { SEN_DX, SEN_DY }, imx327c_60fps, ARRAY_SIZE(imx327c_60fps), 0,
...
//    bool                group_hold_en;
    ...
    int    cam_mode;        // camera mode:
                            //   IMX327C_MODE_1920X1080 (default)
                            //   IMX297_MODE_720X540
...
static const struct regmap_config imx327c_regmap_config = {
    .reg_bits = 16,
    .val_bits = 8,
...
//               const struct imx327c_reg table[])
//{
//    const struct imx327c_reg *reg;
//    int ret;
//
//    for (reg = table; reg->addr != IMX327C_TABLE_END; reg++) {
...
/****** imx327c_read_reg = Read register = 09.2019 ***********************/
static int imx327c_read_reg(struct camera_common_data *s_data, u16 addr, u8 *val)
{
    int err = 0;
    u32 reg_val = 0;
...
/****** imx327c_write_reg = Write register = 09.2019 *********************/
static int imx327c_write_reg(struct camera_common_data *s_data, u16 addr, u8 val)
{
    int err = 0;
...
/****** imx327c_write_table = Write table = 09.2019 **********************/
static int imx327c_write_table(struct imx327c *priv, const imx327c_reg table[])
{
    return regmap_util_write_table_8(priv->s_data->regmap, table, NULL, 0,
        IMX327C_TABLE_WAIT_MS, IMX327C_TABLE_END);
}

/****** imx327c_gpio_set = GPIO set = 09.2019 ****************************/
static void imx327c_gpio_set(struct camera_common_data *s_data,
...
/****** imx327c_set_gain = Set gain = 09.2019 ****************************/
...
//    struct camera_common_data *s_data = tc_dev->s_data;
    ...
    else if (gain > IMX327C_DIGITAL_GAIN_MAX)
      gain = IMX327C_DIGITAL_GAIN_MAX;

//    gain = IMX327C_DIGITAL_GAIN_DEFAULT; // VC: FIXED to 1.00 for now
    priv->digital_gain = gain;

#if TRACE_IMX327C_SET_GAIN
...
//        ret |= reg_write(client, 0x3204, priv->digital_gain & 0xff);
    ...
    regs[1].addr = IMX327C_TABLE_END;
    regs[1].val = 0;

//#if IMX327C_SET_GAIN_GROUP_HOLD
//    if (!priv->group_hold_prev)
//        imx327c_set_group_hold(tc_dev, 1);
//#endif

    err = imx327c_write_table(priv, regs);
    if (err)
        goto fail;
...
//    if ((reg & 0x1f) == 0x01)
//        imx327c_s_stream(&priv->subdev, 1);
...
/****** imx327c_set_exposure = Set exposure = 09.2019 ********************/
static int imx327c_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
...
//    dev_err(dev, "%s: mode->control_properties:\n", __func__);
    ...
    if(priv->exposure_time < IMX327C_DIGITAL_EXPOSURE_MIN) priv->exposure_time = IMX327C_DIGITAL_EXPOSURE_MIN;
    if(priv->exposure_time > IMX327C_DIGITAL_EXPOSURE_MAX) priv->exposure_time = IMX327C_DIGITAL_EXPOSURE_MAX;
...
//      dev_info(dev, "SHS = %d \n",exposure);
#if TRACE_IMX327C_SET_EXPOSURE
      dev_err(dev, "%s(): exposure_time=%d, SHS=0x%x (%d)\n", __func__, priv->exposure_time, exposure, exposure);
//      dev_err(dev, "%s(): TEGRA_CAMERA_CID_EXPOSURE=%d\n", __func__, TEGRA_CAMERA_CID_EXPOSURE);
//      dev_err(dev, "%s(): VIDIOC_S_CTRL=%ld\n", __func__, VIDIOC_S_CTRL);
#endif
...
//        ret |= reg_write(client, 0x3018,  exposure        & 0xff);
    ...
    if (err)
    {
      dev_err(dev, "%s: imx327c_write_table() error=%d\n", __func__, err);
    }
...
/****** imx327c_set_frame_rate = Set frame rate = 09.2019 ****************/
static int imx327c_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
...
//
//#if TRACE_IMX327C_SET_FRAME_RATE
//    dev_err(dev, "%s: frame_rate=%d: frame_length=%d\n", __func__, (int)val,frame_length);
//#endif
//
//    if(frame_length < IMX327C_MIN_FRAME_LENGTH) frame_length = IMX327C_MIN_FRAME_LENGTH;
//    if(frame_length > IMX327C_MAX_FRAME_LENGTH) frame_length = IMX327C_MAX_FRAME_LENGTH;
//
////      err = imx327c_set_frame_length(tc_dev, frame_length);
////      if (err)
////         goto fail;
//
...
/****** imx327c_set_group_hold = Set group hold = 04.2020 ****************/
static int imx327c_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
    /* imx327c does not support group hold */
    return 0;
}
...
static struct tegracam_ctrl_ops imx327c_ctrl_ops = {
    .numctrls = ARRAY_SIZE(ctrl_cid_list),
    .ctrl_cid_list = ctrl_cid_list,
    .set_gain = imx327c_set_gain,
    .set_exposure = imx327c_set_exposure,
    .set_frame_rate = imx327c_set_frame_rate,  // imx327c_set_frame_length,
    .set_group_hold = imx327c_set_group_hold,

};

/****** imx327c_power_on = Power on = 09.2019 ****************************/
static int imx327c_power_on(struct camera_common_data *s_data)
{
#define TRACE_IMX327C_POWER_ON   0   /* DDD - imx327c_power_on() - trace */
...
//    }
    ...
    if (pw->avdd) {
        ...
        if (err)
            goto imx327c_avdd_fail;
    }

    if (pw->iovdd) {
        err = regulator_enable(pw->iovdd);
        if (err)
            goto imx327c_iovdd_fail;
    }
...
skip_power_seqn:
    ...
    if (gpio_is_valid(pw->pwdn_gpio))
        imx327c_gpio_set(s_data, pw->pwdn_gpio, 1);
...
//    msleep(20);
    ...
    if (gpio_is_valid(pw->reset_gpio))
        imx327c_gpio_set(s_data, pw->reset_gpio, 1);
...
/****** imx327c_power_off = Power off = 09.2019 **************************/
static int imx327c_power_off(struct camera_common_data *s_data)
{

#define TRACE_IMX327C_POWER_OFF  0   /* DDD - imx327c_power_off() - trace */
    ...
    usleep_range(21, 25);
    if (gpio_is_valid(pw->pwdn_gpio))
        imx327c_gpio_set(s_data, pw->pwdn_gpio, 0);
    usleep_range(1, 2);
    if (gpio_is_valid(pw->reset_gpio))
        imx327c_gpio_set(s_data, pw->reset_gpio, 0);
...
/****** imx327c_power_put = Power put = 09.2019 **************************/
static int imx327c_power_put(struct tegracam_device *tc_dev)
{
    struct camera_common_data *s_data = tc_dev->s_data;
    struct camera_common_power_rail *pw = s_data->power;
//    struct camera_common_pdata *pdata = s_data->pdata;
...
/****** imx327c_power_get = Power get = 09.2019 **************************/
static int imx327c_power_get(struct tegracam_device *tc_dev)
{

#define TRACE_IMX327C_POWER_GET 0   /* DDD - imx327c_power_get() - trace */
#define RESET_GPIO_ENB          0   /* CCC - imx327c_power_get() - enable reset_gpio code */
...
/****** imx327c_parse_dt = Parse DT = 09.2019 ****************************/
static struct camera_common_pdata *imx327c_parse_dt(
    struct tegracam_device *tc_dev)
{

#define TRACE_IMX327C_PARSE_DT   1   /* DDD - imx327c_parse_dt() - trace */

// IMX219 : no parse clocks
//#define PARSE_CLOCKS        0   /* CCC - imx327c_parse_dt() - parse clocks */
//#define PARSE_GPIOS         0   /* CCC - imx327c_parse_dt() - parse GPIOss */
...
//    match = of_match_device(imx327c_of_match, dev);
//    if (!match) {
...
/****** imx327c_dump_regs = Dump IMX registers = 10.2019 *****************/
int imx327c_dump_regs(struct imx327c *priv)
{

#define REG_WRITE_TEST  0   /* DDD - imx327c_dump_regs - reg write test */
...
#if REG_WRITE_TEST
    low = 0x40;
    dev_err(dev, "%s(): IMX327C: Write WINMODE = 0x%x\n", __func__, (int) low);
    imx327c_write_reg(s_data, addr, low);
#endif

    imx327c_read_reg(s_data, addr, &low);
    sval = (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: WINMODE = 0x%x\n", __func__, sval);
...
// FRSEL[1:0]      = 0x3009[1:0] : frame rate select: 1=60fps, 2=30fps
    ...
    imx327c_read_reg(s_data, addr, &low);
    sval = (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: FRSEL   = 0x%x\n", __func__, sval);
...
// WINWH (DX) H,L  = 0x3043,0x3042
    addr = 0x3043;
    imx327c_read_reg(s_data, addr, &high);
    addr = 0x3042;
    imx327c_read_reg(s_data, addr, &low);
    sval = (high << 8) | (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: sensor width = 0x%x (%d)\n", __func__, sval,sval);
...
// WINWV (DY) H,L  = 0x303F,0x303E
    ...
    imx327c_read_reg(s_data, addr, &low);
    sval = (high << 8) | (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: sensor height = 0x%x (%d)\n", __func__, sval,sval);

// WINWV_OB[3:0]   = 0x303A
    addr = 0x303A;
    imx327c_read_reg(s_data, addr, &low);
    sval = (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: WINWV_OB = 0x%x (%d)\n", __func__, sval,sval);
...
// X_OUT_SIZE[12:0] H,L = 0x3473,0x3472 = Horizontal (H) direction effective pixel width setting.
    ...
    imx327c_read_reg(s_data, addr, &low);
    sval = (high << 8) | (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: X_OUT_SIZE = 0x%x (%d)\n", __func__, sval,sval);
...
// Y_OUT_SIZE H,L [12:0] = 0x3419,0x3418 = Vertical direction effective pixels
    ...
    imx327c_read_reg(s_data, addr, &low);
    sval = (high << 8) | (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: Y_OUT_SIZE = 0x%x (%d)\n", __func__, sval,sval);
...
// HMAX[15:0] H,L =  0x301D,0x301C
    ...
    imx327c_read_reg(s_data, addr, &low);
    sval = (high << 8) | (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: HMAX = 0x%x (%d)\n", __func__, sval,sval);

// VMAX[17:0] H,M,L =  0x301A,0x3019,0x3018
    addr = 0x301A;
    imx327c_read_reg(s_data, addr, &high);
    addr = 0x3019;
    imx327c_read_reg(s_data, addr, &mid);
    addr = 0x3018;
    imx327c_read_reg(s_data, addr, &low);
    sval = (high << 16) | (mid << 8) | (low & 0xFF);
    dev_err(dev, "%s(): IMX327C: VMAX = 0x%x (%d)\n", __func__, sval,sval);

//// embedded data line control register
//    addr = 0xBCF1;
////    low = 2;
////    imx327c_write_reg(s_data, addr, low);
////    dev_err(dev, "%s(): IMX327c: Set: embedded data line control = 2\n", __func__);
//    imx327c_read_reg(s_data, addr, &high);
//    dev_err(dev, "%s(): IMX327C: embedded data line control = 0x%x\n", __func__, (int)high);

//done:
    return err;
}

/****** imx327c_set_mode = Set mode = 09.2019 ****************************/
static int imx327c_set_mode(struct tegracam_device *tc_dev)
{

#define TRACE_IMX327C_SET_MODE   1   /* DDD - imx327c_set_mode() - trace */
#define DUMP_IMX327C_REGS1       0   /* DDD - imx327c_set_mode() - dump sensor regs aft set mode */



//#if !SKIP_IMX327C_SET_MODE
    struct imx327c *priv = (struct imx327c *)tegracam_get_privdata(tc_dev);
//#endif
    struct camera_common_data *s_data = tc_dev->s_data;
    struct device *dev = tc_dev->dev;
...
// Get camera mode:
    ...
    {
      priv->cam_mode = IMX327C_MODE_1920X1080;
    }
//    else
//    {
...
//    if(priv->cam_mode != IMX327C_MODE_1920X1080)
    {
      err = imx327c_write_table(priv, imx327c_mode_table[priv->cam_mode]);
      if (err)
      {
        dev_err(dev, "%s(): imx327c_write_table() error=%d\n", __func__, err);
//        goto exit;
      ...
      {
#if TRACE_IMX327C_SET_MODE
//        dev_err(dev, "%s(): imx327c_write_table() OK, cam_mode=%d\n", __func__, priv->cam_mode);
#endif
      }
    }
...
//    imx327c_dump_regs(priv);
}
#endif

#if DUMP_IMX327C_REGS1
{
...
/****** imx327c_start_streaming = Start streaming = 09.2019 **************/
static int imx327c_start_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX327C_START_STREAMING       1   /* DDD - imx327c_start_streaming() - trace */
#define IMX327C_START_STREAMING_SET_CTRLS   1   /* CCC - imx327c_start_streaming() - set controls */
...
#if IMX327C_START_STREAMING_SET_CTRLS
      imx327c_set_gain(tc_dev, priv->digital_gain);
      imx327c_set_exposure(tc_dev, priv->exposure_time);
      imx327c_set_frame_rate(tc_dev, priv->frame_rate);
//      imx327c_set_frame_length(tc_dev, priv->frame_length);
#endif

#if TRACE_IMX327C_START_STREAMING
//    dev_info(dev, "%s(): Set frame_length=%d\n", __func__, IMX327C_DEFAULT_FRAME_LENGTH);
#endif

#if TRACE_IMX327C_START_STREAMING
//    dev_info(dev, "%s(): Set gain=%d\n", __func__, IMX327C_DEFAULT_GAIN);
#endif
...
//    mutex_lock(&priv->streaming_lock);
    err = imx327c_write_table(priv, imx327c_mode_table[IMX327C_MODE_START_STREAM]);
    if (err) {
...
/****** imx327c_stop_streaming = Stop streaming = 09.2019 ****************/
static int imx327c_stop_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX327C_STOP_STREAMING      1    /* DDD - imx327c_stop_streaming() - trace */
#define IMX327C_STOP_STREAMING_ENB_RESET    0    /* CCC - imx327c_stop_streaming() - enable VC MIPI reset */
...
#if TRACE_IMX327C_STOP_STREAMING
//    dev_err(dev, "%s():\n", __func__);
#endif

    err = imx327c_write_table(priv, imx327c_mode_table[IMX327C_MODE_STOP_STREAM]);
    if (err)
    {
//#if TRACE_IMX327C_STOP_STREAMING
      dev_err(dev, "%s(): imx327c_write_table() err=%d\n", __func__, err);
//#endif
//        mutex_unlock(&priv->streaming_lock);
        goto exit;
    }
...
#if IMX327C_STOP_STREAMING_ENB_RESET  // [[[
        err = vc_mipi_reset(tc_dev, sensor_mode);
        if(err)
        {
...
static struct camera_common_sensor_ops imx327c_common_ops = {
    .numfrmfmts = ARRAY_SIZE(imx327c_frmfmt),
    .frmfmt_table = imx327c_frmfmt,
    .power_on = imx327c_power_on,
    .power_off = imx327c_power_off,
    .write_reg = imx327c_write_reg,
    .read_reg = imx327c_read_reg,
    .parse_dt = imx327c_parse_dt,
    .power_get = imx327c_power_get,
    .power_put = imx327c_power_put,
    .set_mode = imx327c_set_mode,
    .start_streaming = imx327c_start_streaming,
    .stop_streaming = imx327c_stop_streaming,
};

/****** imx327c_video_probe = Video probe = 09.2019 **********************/
static int imx327c_video_probe(struct i2c_client *client)
{
    u16 model_id;
    u32 lot_id=0;
    u16 chip_id=0;
...
/****** imx327c_probe_vc_rom = Probe VC ROM = 09.2019 ********************/
static struct i2c_client * imx327c_probe_vc_rom(struct i2c_adapter *adapter, u8 addr)
{
...
/****** imx327c_board_setup = Board setup = 09.2019 **********************/
static int imx327c_board_setup(struct imx327c *priv)
{

#define TRACE_IMX327C_BOARD_SETUP  1   /* DDD - imx327c_board_setup() - trace */
#define DUMP_CTL_REG_DATA          0   /* DDD - imx327c_board_setup() - dump module control registers 0x100-0x108 (I2C addr=0x10) */
#define DUMP_HWD_DESC_ROM_DATA     0   /* DDD - imx327c_board_setup() - dump Hardware Desriptor ROM data (I2C addr=0x10) */
#define DUMP_IMX327C_REGS          0   /* DDD - imx327c_board_setup() - dump IMX327C regs */
#define DUMP_V4L_PARAMS            1   /* DDD - imx327c_board_setup() - dump V4L params */
#define MOD_ID                0x0327   /* CCC - imx290_board_setup() - module id., 0=off: don't check */
...
//    priv->rom = i2c_new_dummy(adapter,0x10);
    priv->rom = imx327c_probe_vc_rom(adapter,0x10);
...
//          dev_err(&client->dev, "0x%04x: %02x\n", addr, reg_val[i]);
    ...
    ret = imx327c_video_probe(client);
    if (ret < 0)
    {
        dev_err(dev, "%s(): imx327c_video_probe() error=%d\n", __func__, ret);
        err = -EIO;
        goto done;
    }
...
/****** imx327c_open = Open device = 09.2019 *****************************/
static int imx327c_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
...
static const struct v4l2_subdev_internal_ops imx327c_subdev_internal_ops = {
    .open = imx327c_open,
};

static const struct of_device_id imx327c_of_match[] = {
    { .compatible = "nvidia,imx327c", },
    { },
};
MODULE_DEVICE_TABLE(of, imx327c_of_match);

/****** imx327c_probe = Probe = 09.2019 **********************************/
static int imx327c_probe(struct i2c_client *client,
...
//    dev_info(dev, "%s(): Probing v4l2 sensor at addr 0x%0x\n", __func__, client->addr); // , __DATE__, __TIME__);
    ...
    priv = devm_kzalloc(dev,
                sizeof(struct imx327c), GFP_KERNEL);
    if (!priv)
        return -ENOMEM;
    ...
    tc_dev->dev = dev;
    strncpy(tc_dev->name, "imx327c", sizeof(tc_dev->name));
    tc_dev->dev_regmap_config = &imx327c_regmap_config;
    tc_dev->sensor_ops = &imx327c_common_ops;
    tc_dev->v4l2sd_internal_ops = &imx327c_subdev_internal_ops;
    tc_dev->tcctrl_ops = &imx327c_ctrl_ops;
    ...
    priv->exposure_time = IMX327C_DIGITAL_EXPOSURE_DEFAULT;
    priv->frame_rate    = IMX327C_FRAME_RATE_DEFAULT;
//    priv->frame_length = IMX327C_DEFAULT_FRAME_LENGTH;
//    priv->sensor_ext_trig = 0;    // ext. trigger flag: 0=no, 1=yes

    priv->sensor_ext_trig = 0;    // ext. trigger flag: 0=no, 1=yes
//    priv->sen_clk = 54000000;     // clock-frequency: default=54Mhz imx183=72Mhz
    priv->sen_clk = IMX327_CLOCK_FREQUENCY;
    priv->flash_output = flash_output;

    dev_err(dev, "%s(): Detected imx327c sensor - %s/%s\n", __func__, __DATE__, __TIME__);

//    switch(priv->model)
//    {
//      case IMX327C_MODEL_MONOCHROME:
//        priv->cam_mode = IMX327C_MODE_1920X1080;
//        dev_err(dev, "%s(): Detected imx327c sensor\n", __func__); // , __DATE__, __TIME__);
//        break;
//
//      case IMX327C_MODEL_COLOR:
//        priv->cam_mode = IMX327C_MODE_1920X1080;
//        dev_err(dev, "%s(): Detected imx327cc sensor\n", __func__); // , __DATE__, __TIME__);
//        break;
//
...
//
//      default:
//        dev_err(dev, "%s(): Detected inknown imx327c/297 sensor\n", __func__); // , __DATE__, __TIME__);
//        break;
//    }
...
/****** imx327c_remove = Remove = 09.2019 ********************************/
static int imx327c_remove(struct i2c_client *client)
{
    struct camera_common_data *s_data = to_camera_common_data(&client->dev);
    struct imx327c *priv = (struct imx327c *)s_data->priv;
...
static const struct i2c_device_id imx327c_id[] = {
    { "imx327c", 0 },
    { }
};

MODULE_DEVICE_TABLE(i2c, imx327c_id);



/****** i2c_driver = I2C driver = 09.2019 *******************************/
static struct i2c_driver imx327c_i2c_driver = {
    .driver = {
        .name = "imx327c",
        .owner = THIS_MODULE,
        .of_match_table = of_match_ptr(imx327c_of_match),
    },
    .probe = imx327c_probe,
    .remove = imx327c_remove,
    .id_table = imx327c_id,
};
module_i2c_driver(imx327c_i2c_driver);
