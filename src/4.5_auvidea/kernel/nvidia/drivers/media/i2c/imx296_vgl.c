...
//#define DEV_PRINT         dev_err
//#define DEV_PRINT         dev_warn
#define DEV_PRINT         dev_info
//#define DEV_PRINT         dev_dbg
...
#define IMX296_DIGITAL_EXPOSURE_MAX     16000000
...
#if DUMP_CTL_REGS
    DEV_PRINT(&client->dev, "0x10: R[0x%04x]=0x%04x\n", (int)addr, (int)data);
#endif
...
//        ret |= reg_write(client, 0x308d,  exposure        & 0xff);
    ...
    else
    {
        ...
        dev_info(dev, "VMAX = %d \n",exposure);
#if TRACE_IMX296_EXPOSURE
    DEV_PRINT(dev, "%s(): exposure_time=%d, VMAX=0x%x (%d)\n", __func__, priv->exposure_time, exposure, exposure);
#endif
...
//    DEV_PRINT(dev, "%s: mode->control_properties:\n", __func__);
    DEV_PRINT(dev, "%s: min_exp_time,max_exp_time=%d,%d default_exp_time=%d\n", __func__,
...
/*............. HW trigger mode */
    ...
    if(priv->sensor_ext_trig)
    {
      ...
      DEV_PRINT(dev, "%s(): exposure_time=%d: trigger mode exposure=%llu (0x%llx)\n", __func__, priv->exposure_time, exposure, exposure);
#endif
...
#if TRACE_IMX296_SET_EXPOSURE
      DEV_PRINT(dev, "%s(): Set exposure_time=%d\n", __func__, priv->exposure_time);
#endif
...
//
//#if TRACE_IMX296_SET_EXPOSURE
//    DEV_PRINT(dev, "%s(): exposure_time=%d, EXP=0x%x\n", __func__, exposure_time, exposure);
//#endif
//
...
//    if (err)
//    {
//      DEV_PRINT(dev, "%s: imx296_write_table() error=%d\n", __func__, err);
//    }
...
//    DEV_PRINT(dev, "%s: frame_rate=%d: frame_length=%d\n", __func__, (int)val,frame_length);
//#endif
//
...
// do we need reset-gpios ?
    ...
    if (gpio < 0) {
        ...
        dev_warn(dev, "reset-gpios not found\n");
        goto error;
    }
...
// IMX219
    ...
    if (err)
        dev_warn(dev, "%s(): mclk name not present, assume sensor driven externally\n", __func__);
    ...
    if (err)
        dev_warn(dev, "%s(): avdd, iovdd and/or dvdd reglrs. not present, assume sensor powered independently\n", __func__);
...
//............. Read flash output enable from DT
    ...
    if (err)
    {
        DEV_PRINT(dev, "%s(): flash-output not present in DT, def=%d\n", __func__, flash_output);
    }
    else
    {
      flash_output = val;
      DEV_PRINT(dev, "%s(): flash-output=%d\n", __func__, flash_output);
    }
...
#if DUMP_SENSOR_MODE
{
    struct sensor_properties *sensor_props = &s_data->sensor_props;
    int num_modes = sensor_props->num_modes;
    DEV_PRINT(dev, "%s: sensor_mode_id=%d use_sensor_mode_id=%d num_modes=%d\n", __func__, s_data->sensor_mode_id, s_data->use_sensor_mode_id, num_modes);
}
#endif
...
//};
    ...
    DEV_PRINT(dev, "%s: mode=%d mode_prop_idx=%d\n", __func__, s_data->mode, s_data->mode_prop_idx);
//    DEV_PRINT(dev, "%s: sensor_mode_id=%d use_sensor_mode_id=%d\n", __func__, s_data->sensor_mode_id, s_data->use_sensor_mode_id);
    DEV_PRINT(dev, "%s: mclk_freq=%d pixel_clock=%u cil_settletime=%d discontinuous_clk=%d\n", __func__,
         mclk_freq, (unsigned) pixel_clock, cil_settletime, discontinuous_clk);
    DEV_PRINT(dev, "%s: width,height,line_length=%d,%d,%d pixel_format=0x%x embedded_metadata_height=%d\n", __func__,
...
// Change VC MIPI trigger mode
    if(priv->sensor_mode != sensor_mode)
    {
      ...
      DEV_PRINT(dev, "%s(): New sensor_mode=%d (0=free run,1=ext. trigger) sensor_ext_trig=%d\n", __func__, sensor_mode, priv->sensor_ext_trig);
#endif
...
//      if(tcam_mode >= 0)
//      {
//        DEV_PRINT(dev, "%s(): Trig fix mode: fmt_width,fmt_height=%d,%d pix_fmt=0x%x '%c%c%c%c', cam_mode=%d (0=296, 1=297), err=%d\n", __func__,
...
//        goto exit;
      ...
      {
#if TRACE_IMX296_SET_MODE
        DEV_PRINT(dev, "%s(): imx296_write_table() OK, cam_mode=%d\n", __func__, priv->cam_mode);
#endif
      }

#if TRACE_IMX296_SET_MODE
{
      DEV_PRINT(dev, "%s(): fmt_width,fmt_height=%d,%d pix_fmt=0x%x '%c%c%c%c', cam_mode=%d (0=296, 1=297), err=%d\n", __func__,
                        s_data->fmt_width, s_data->fmt_height,
                        pix_fmt,
                        (char)((pix_fmt      ) & 0xFF),
                        (char)((pix_fmt >>  8) & 0xFF),
...
//        if(err)
//        {
//          DEV_PRINT(dev, "%s(): vc_mipi_reset() error=%d\n", __func__, err);
//          goto exit;
//        }
//#endif

#if TRACE_IMX296_START_STREAMING
       DEV_PRINT(dev, "%s(): sensor_ext_trig=%d, exposure=%llu (0x%llx)\n", __func__, priv->sensor_ext_trig, exposure, exposure);
#endif
...
//    DEV_PRINT(dev, "%s():\n", __func__);
    ...
    {
//#if TRACE_IMX296_STOP_STREAMING
      DEV_PRINT(dev, "%s(): imx296_write_table() err=%d\n", __func__, err);
//#endif
//        mutex_unlock(&priv->streaming_lock);
        goto exit;
    }
...
//        DEV_PRINT(dev, "%s(): Disabled VC sensor reset\n", __func__);
//#else  // ]]] [[[
...
//};
    ...
    DEV_PRINT(dev, "%s: mclk_freq=%d pixel_clock=%d cil_settletime=%d discontinuous_clk=%d\n", __func__,
                        mclk_freq, pixel_clock, cil_settletime, discontinuous_clk);
    DEV_PRINT(dev, "%s: width,height,line_length=%d,%d,%d pix_fmt=0x%x '%c%c%c%c' embedded_metadata_height=%d\n", __func__,
                        (int)mode->image_properties.width,
                        (int)mode->image_properties.height,
                        (int)mode->image_properties.line_length,
                        pix_fmt,
...
//        static int i=1;
        ...
        DEV_PRINT(&client->dev, "%s(): Dump Hardware Descriptor ROM data:\n", __func__);
#endif
...
#if DUMP_HWD_DESC_ROM_DATA      /* [[[ - dump Hardware Desriptor ROM data */
{
          ...
          {
            sval |= (int)reg << 8;
            DEV_PRINT(&client->dev, "addr=0x%04x reg=0x%04x\n",addr+0x1000-1,sval);
          }
          else
          {
            sval = reg;
        ...
        DEV_PRINT(&client->dev, "%s(): VC FPGA found!\n", __func__);

        DEV_PRINT(&client->dev, "[ MAGIC  ] [ %s ]\n",
                priv->rom_table.magic);

        DEV_PRINT(&client->dev, "[ MANUF. ] [ %s ] [ MID=0x%04x ]\n",
                priv->rom_table.manuf,
                priv->rom_table.manuf_id);

        DEV_PRINT(&client->dev, "[ SENSOR ] [ %s %s ]\n",
                priv->rom_table.sen_manuf,
                priv->rom_table.sen_type);

        DEV_PRINT(&client->dev, "[ MODULE ] [ ID=0x%04x ] [ REV=0x%04x ]\n",
                priv->rom_table.mod_id,
                priv->rom_table.mod_rev);

        DEV_PRINT(&client->dev, "[ MODES  ] [ NR=0x%04x ] [ BPM=0x%04x ]\n",
...
#if DUMP_CTL_REG_DATA
{
        ...
        DEV_PRINT(&client->dev, "%s(): Module controller registers (0x10):\n", __func__);
...
//          DEV_PRINT(&client->dev, "0x%04x: %02x\n", addr, reg_val[i]);
          i++;
        }

        DEV_PRINT(&client->dev, "0x100-103: %02x %02x %02x %02x\n", reg_val[0],reg_val[1],reg_val[2],reg_val[3]);
        DEV_PRINT(&client->dev, "0x104-108: %02x %02x %02x %02x %02x\n", reg_val[4],reg_val[5],reg_val[6],reg_val[7],reg_val[8]);
...
#if TRACE_IMX296_BOARD_SETUP
{
    ...
    DEV_PRINT(&client->dev, "%s(): Success: sensor model=%s, err=%d\n", __func__, sen_models[priv->model], err);
}
#endif
...
//    struct device_node *node = client->dev.of_node;
    ...
    DEV_PRINT(dev, "%s(): Probing v4l2 sensor at addr 0x%0x - %s/%s\n", __func__, client->addr, __DATE__, __TIME__);
...
// set default camera mode
    switch(priv->model)
    {
      case IMX296_MODEL_MONOCHROME:
        priv->cam_mode = IMX296_MODE_1440X1080;
        DEV_PRINT(dev, "%s(): Detected imx296 sensor - %s/%s\n", __func__, __DATE__, __TIME__); // , __DATE__, __TIME__);
        break;

      case IMX296_MODEL_COLOR:
        priv->cam_mode = IMX296_MODE_1440X1080;
        DEV_PRINT(dev, "%s(): Detected imx296c sensor - %s/%s\n", __func__, __DATE__, __TIME__); // , __DATE__, __TIME__);
        break;
...
//        break;
      ...
      default:
        DEV_PRINT(dev, "%s(): Detected non-imx296 sensor - %s/%s\n", __func__, __DATE__, __TIME__); // , __DATE__, __TIME__);
        break;
    }
