...
#define VC_CODE             1   /* CCC - imx183.c - enable code for VC MIPI camera  */
#define IMX183_ENB_MUTEX    0   /* CCC - imx183c - mutex lock/unlock */
#define IMX183_TRIG_MODE    0   /* CCC - imx183c - force sensor ext. trigger mode */
...
//#include <media/imx183.h>

#include "../platform/tegra/camera/camera_gpio.h"
#include "vc_mipi.h"
//#include "imx183.h"
//#include "imx183_mode_tbls.h"
...
#define IMX183_TABLE_WAIT_MS    0
#define IMX183_TABLE_END        1
...
/* In dB*10 */
//#define IMX183_ANALOG_GAIN_MIN          0
//#define IMX183_ANALOG_GAIN_MAX          978
//#define IMX183_ANALOG_GAIN_DEFAULT      0

// PK: just for test, not supported yet
//#define IMX183_MIN_FRAME_LENGTH     100
//#define IMX183_MAX_FRAME_LENGTH     800000
#define IMX183_FRAME_RATE_DEFAULT   30000000   // 30 fps (*1000000)
...
enum {
    IMX183_MODE_5440X3648 = 0,
    IMX183_MODE_START_STREAM,
    IMX183_MODE_STOP_STREAM,
};

#define imx183_reg struct reg_8

static const imx183_reg imx183_start[] = {
    {0x7000, 0x01},         /* mode select streaming on */
    {IMX183_TABLE_END, 0x00}
};

static const imx183_reg imx183_stop[] = {
...
#define IMX183_DY 3648

#define IMX183_TRIG_FIX     0   /* CCC - imx183.c - fix trigger mode problem:                     */
                                /* in trigger mode the sensor produces frame height = IMX183_DY-2 */
                                /* switch between frame format tables:                            */
                                /*   imx183_frmfmt      : in free-run mode                        */
                                /*   imx183_trig_frmfmt : in trigger mode                         */

//#if IMX183_TRIG_FIX // trig mode frame DY:
//  #define IMX183_DY 3648
//#else
//  #define IMX183_DY 3648
//#endif
...
//     - vwinpos (low)  (0x006F) ==> 10
//     - vwidcut (low)  (0x0071) ==> 3648
//     - vwidcut (high) (0x0072) ==> 3648
//
/*
VWIDCUT Setting
--------------
...
IMX183_DY = 3694 – VWIDCUT * 2
VWIDCUT = (3694 – IMX183_DY) / 2

MDSEL3 (0x0006) = 0x20  :  Enable vertical cropping width
MDSEL4 (0x0007) = 0x50  :  Enable vertical arbitrary cropping
*/
...
//#define IMX183_DY 3648
//
//#define VWIDCUT ((3694-IMX183_DY)/2)
//#define MDSEL3   0x20
//
//#define VWINPOS  10
//#define MDSEL4   0x50

static const imx183_reg imx183_mode_5440X3648[] = {
#if 0  // Set vertical cropping width
    { 0x0071, VWIDCUT & 0xFF }, { 0x0072, (VWIDCUT>>8) & 0xFF },   //  VWIDCUT L,H  = 0x0071,0x0072,
    { 0x0006, MDSEL3  & 0xFF },                                    //  MDSEL3  L    = 0x0006
#endif

#if 0   // Set vertical cropping position
    { 0x006F, VWINPOS & 0xFF },                                    //  VWINPOS L,H  = 0x006F, 0x0070,
    { 0x0007, MDSEL4  & 0xFF },                                    //  MDSEL4  L    = 0x0007
#endif
...
static const imx183_reg *imx183_mode_table[] = {
    [IMX183_MODE_5440X3648]    = imx183_mode_5440X3648,
    [IMX183_MODE_START_STREAM] = imx183_start,
    [IMX183_MODE_STOP_STREAM]  = imx183_stop,
};

static const int imx183_20fps[] = {
    20,
};

static const struct camera_common_frmfmt imx183_frmfmt[] = {
    { { IMX183_DX, IMX183_DY }, imx183_20fps, ARRAY_SIZE(imx183_20fps), 0,
      IMX183_MODE_5440X3648 },
};

//#if IMX183_TRIG_FIX   // [[[ - trig mode fix: does not work in free-run
  static const struct camera_common_frmfmt imx183_trig_frmfmt[] = {
    { { IMX183_DX, IMX183_DY-2 }, imx183_20fps, ARRAY_SIZE(imx183_20fps), 0,
      IMX183_MODE_5440X3648 },
};
//#endif  // ]]]
...
static const struct regmap_config imx183_regmap_config = {
    .reg_bits = 16,
    .val_bits = 8,
...
//static char imx183_sensor_model[] = "imx183";
void set_sensor_model (char *model);


//#define VC_MIPI_MODEL       1   /* CCC - imx183.c - enable model code */
//
//#if VC_MIPI_MODEL     /* [[[ */
//#define MAX_MODEL_LEN  64
//static char sensor_model[MAX_MODEL_LEN] = "imx183";
//
...
//               const struct imx183_reg table[])
//{
//    const struct imx183_reg *reg;
//    int ret;
//
//    for (reg = table; reg->addr != IMX183_TABLE_END; reg++) {
...
/****** imx183_read_reg = Read register = 02.2020 ***********************/
static int imx183_read_reg(struct camera_common_data *s_data, u16 addr, u8 *val)
{
    int err = 0;
    u32 reg_val = 0;
...
/****** imx183_write_reg = Write register = 02.2020 *********************/
static int imx183_write_reg(struct camera_common_data *s_data, u16 addr, u8 val)
{
    int err = 0;
...
/****** imx183_write_table = Write table = 02.2020 **********************/
static int imx183_write_table(struct imx183 *priv, const imx183_reg table[])
{
    return regmap_util_write_table_8(priv->s_data->regmap, table, NULL, 0,
        IMX183_TABLE_WAIT_MS, IMX183_TABLE_END);
}
...
static int vc_mipi_common_reg_read(struct camera_common_data *s_data, u16 addr)
{
    ...
    err = imx183_read_reg(s_data, addr, &val);
    if(err) return -1;
...
{
    return imx183_write_reg(s_data, addr, val);
}


/****** imx183_gpio_set = GPIO set = 02.2020 ****************************/
static void imx183_gpio_set(struct camera_common_data *s_data,
...
/****** imx183_set_gain = Set gain = 02.2020 ****************************/
static int imx183_set_gain(struct tegracam_device *tc_dev, s64 val)
{

#define TRACE_IMX183_SET_GAIN       1   /* DDD - imx183_set_gain() - trace */
//#define IMX183_SET_GAIN_GROUP_HOLD   0   /* CCC - imx183_set_gain() - enable group hold bef reg write */

    struct camera_common_data *s_data = tc_dev->s_data;
    struct imx183 *priv = (struct imx183 *)tc_dev->priv;
...
//    s64 gain = val;
    ...
    if ( priv->digital_gain < IMX183_DIGITAL_GAIN_MIN)
       priv->digital_gain = IMX183_DIGITAL_GAIN_MIN;
    else if ( priv->digital_gain > IMX183_DIGITAL_GAIN_MAX)
       priv->digital_gain = IMX183_DIGITAL_GAIN_MAX;

//    gain = IMX183_DIGITAL_GAIN_DEFAULT; // VC: FIXED to 1.00 for now

#if TRACE_IMX183_SET_GAIN
...
//#define GAIN_L  0x000A
//#define GAIN_H  0x0009
...
//    regs[2].addr = IMX183_TABLE_END;
//    regs[2].val = 0;
//
//
//
////#if IMX183_SET_GAIN_GROUP_HOLD
////    if (!priv->group_hold_prev)
////        imx183_set_group_hold(tc_dev, 1);
////#endif
//
//    err = imx183_write_table(priv, regs);
//    if (err)
//        goto fail;
...
/****** imx183_set_exposure = Set exposure = 02.2020 ********************/
static int imx183_set_exposure(struct tegracam_device *tc_dev, s64 val)
{

//static int imx_exposure_imx183(struct imx *priv, s32 EXPOSURE_TIME_MIN_183, s32 EXPOSURE_TIME_MIN2_183, s32 EXPOSURE_TIME_MAX_183, s32 NRLINES_183,
//                                                        s32 TOFFSET_183, s32 H1PERIOD_183, s32 VMAX_183)

#define TRACE_IMX183_SET_EXPOSURE   1   /* DDD - imx183_set_exposure() - trace */
#define DUMP_EXPOSURE_PARAMS        0   /* DDD - imx183_set_exposure() - dump DT exposure params */

    struct device *dev = tc_dev->dev;
    struct imx183 *priv = tc_dev->priv;
...
//    if(priv->exposure_time < IMX183_DIGITAL_EXPOSURE_MIN) priv->exposure_time = IMX183_DIGITAL_EXPOSURE_MIN;
//    if(priv->exposure_time > IMX183_DIGITAL_EXPOSURE_MAX) priv->exposure_time = IMX183_DIGITAL_EXPOSURE_MAX;

    if (priv->exposure_time < EXPOSURE_TIME_MIN_183)
        priv->exposure_time = EXPOSURE_TIME_MIN_183;

    if (priv->exposure_time > EXPOSURE_TIME_MAX_183)
        priv->exposure_time = EXPOSURE_TIME_MAX_183;
...
//      dev_info(dev, "hmax = %d \n",hmax);
      ...
      if (priv->exposure_time < EXPOSURE_TIME_MIN2_183)
      {
        // exposure = (NumberOfLines - exp_time / 1Hperiod + toffset / 1Hperiod )
        // shutter = {VMAX - SHR}*HMAX + 209(157) clocks

        //exposure = (NRLINES_183  -  ((int)(priv->exposure_time) * 16384 - TOFFSET_183)/H1PERIOD_183);
        exposure = (vMax -  ((int)(priv->exposure_time) * 16384 - TOFFSET_183)/H1PERIOD_183);
//        dev_info(dev, "SHS = %d \n",exposure);
        dev_info(dev, "SHS = %d , VMAX = %d \n", exposure, vMax);
      ...
      {
        // exposure = 5 + ((unsigned long long)(priv->exposure_time * 16384) - TOFFSET_183)/H1PERIOD_183;
        u64 divresult;
...
//      dev_info(dev, "VMAX = %d \n",exposure);
        ...
        if(sen_reg(priv, EXPOSURE_LOW))
            ret |= vc_mipi_common_reg_write(s_data, sen_reg(priv, EXPOSURE_LOW), 0x05);

//      ret |= vc_mipi_common_reg_write(s_data, 0x7006, (exposure >> 16) & 0x07);
        ret |= vc_mipi_common_reg_write(s_data, 0x7006, (exposure >> 16) & 0xff);
...
/****** imx183_set_frame_rate = Set frame rate = 02.2020 ****************/
static int imx183_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{

#define TRACE_IMX183_SET_FRAME_RATE     0   /* DDD - imx183_set_frame_rate() - trace */
...
//
//#if TRACE_IMX183_SET_FRAME_RATE
//    dev_err(dev, "%s: frame_rate=%d: frame_length=%d\n", __func__, (int)val,frame_length);
//#endif
//
//    if(frame_length < IMX183_MIN_FRAME_LENGTH) frame_length = IMX183_MIN_FRAME_LENGTH;
//    if(frame_length > IMX183_MAX_FRAME_LENGTH) frame_length = IMX183_MAX_FRAME_LENGTH;
//
////      err = imx183_set_frame_length(tc_dev, frame_length);
////      if (err)
////         goto fail;
//
...
/****** imx183_set_group_hold = Set group hold = 04.2020 ****************/
static int imx183_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
    /* imx183 does not support group hold */
    return 0;
}
...
static struct tegracam_ctrl_ops imx183_ctrl_ops = {
    .numctrls = ARRAY_SIZE(ctrl_cid_list),
    .ctrl_cid_list = ctrl_cid_list,
    .set_gain = imx183_set_gain,
    .set_exposure = imx183_set_exposure,
    .set_frame_rate = imx183_set_frame_rate,  // imx183_set_frame_length,
    .set_group_hold = imx183_set_group_hold,

};

/****** imx183_power_on = Power on = 02.2020 ****************************/
static int imx183_power_on(struct camera_common_data *s_data)
{
#define TRACE_IMX183_POWER_ON   0   /* DDD - imx183_power_on() - trace */
...
//    }
    ...
    if (pw->avdd) {
        ...
        if (err)
            goto imx183_avdd_fail;
    }

    if (pw->iovdd) {
        err = regulator_enable(pw->iovdd);
        if (err)
            goto imx183_iovdd_fail;
    }
...
skip_power_seqn:
    ...
    if (gpio_is_valid(pw->pwdn_gpio))
        imx183_gpio_set(s_data, pw->pwdn_gpio, 1);
...
//    msleep(20);
    ...
    if (gpio_is_valid(pw->reset_gpio))
        imx183_gpio_set(s_data, pw->reset_gpio, 1);
...
/****** imx183_power_off = Power off = 02.2020 **************************/
static int imx183_power_off(struct camera_common_data *s_data)
{

#define TRACE_IMX183_POWER_OFF  0   /* DDD - imx183_power_off() - trace */
    ...
    usleep_range(21, 25);
    if (gpio_is_valid(pw->pwdn_gpio))
        imx183_gpio_set(s_data, pw->pwdn_gpio, 0);
    usleep_range(1, 2);
    if (gpio_is_valid(pw->reset_gpio))
        imx183_gpio_set(s_data, pw->reset_gpio, 0);
...
/****** imx183_power_put = Power put = 02.2020 **************************/
static int imx183_power_put(struct tegracam_device *tc_dev)
{
    struct camera_common_data *s_data = tc_dev->s_data;
    struct camera_common_power_rail *pw = s_data->power;
//    struct camera_common_pdata *pdata = s_data->pdata;
...
/****** imx183_power_get = Power get = 02.2020 **************************/
static int imx183_power_get(struct tegracam_device *tc_dev)
{

#define TRACE_IMX183_POWER_GET  0   /* DDD - imx183_power_get() - trace */
#define RESET_GPIO_ENB          0   /* CCC - imx183_power_get() - enable reset_gpio code */
...
/****** imx183_parse_dt = Parse DT = 02.2020 ****************************/
static struct camera_common_pdata *imx183_parse_dt(
    struct tegracam_device *tc_dev)
{

#define TRACE_IMX183_PARSE_DT   1   /* DDD - imx183_parse_dt() - trace */

// IMX219 : no parse clocks
//#define PARSE_CLOCKS        0   /* CCC - imx183_parse_dt() - parse clocks */
//#define PARSE_GPIOS         0   /* CCC - imx183_parse_dt() - parse GPIOss */

    struct device *dev = tc_dev->dev;
    struct device_node *node = dev->of_node;
    struct camera_common_pdata *board_priv_pdata;
//    const struct of_device_id *match;
//    struct imx183 *priv = (struct imx183 *)tegracam_get_privdata(tc_dev);
...
//    match = of_match_device(imx183_of_match, dev);
//    if (!match) {
...
#define TRACE_VC_MIPI_RESET     1   /* DDD - vc_mipi_reset() - trace */
    ...
    i = 1;   // reset try counter

#if TRACE_VC_MIPI_RESET
        dev_info(dev, "%s(): VC_SEN_MODE=%d PowerOn STATUS=0x%02x try=%d err=%d\n",__func__, sen_mode,reg,i, err);
#endif
    mdelay(500);
...
///****** imx183_dump_regs = Dump IMX registers = 02.2020 *****************/
//int imx183_dump_regs(struct imx183 *priv)
//{
//
//#define REG_WRITE_TEST  0   /* DDD - imx183_dump_regs - reg write test */
//
//    struct camera_common_data *s_data = priv->s_data;
//
////    struct camera_common_pdata *pdata = s_data->pdata;
...
//#if REG_WRITE_TEST
//    low = 0x40;
//    dev_err(dev, "%s(): IMX183: Write WINMODE = 0x%x\n", __func__, (int) low);
//    imx183_write_reg(s_data, addr, low);
//#endif
//
//    imx183_read_reg(s_data, addr, &low);
//    sval = (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: WINMODE = 0x%x\n", __func__, sval);
//
//
//
...
//    imx183_read_reg(s_data, addr, &low);
//    sval = (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: FRSEL   = 0x%x\n", __func__, sval);
//
...
//    addr = 0x3043;
//    imx183_read_reg(s_data, addr, &high);
//    addr = 0x3042;
//    imx183_read_reg(s_data, addr, &low);
//    sval = (high << 8) | (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: sensor width = 0x%x (%d)\n", __func__, sval,sval);
//
//
//
...
//    imx183_read_reg(s_data, addr, &low);
//    sval = (high << 8) | (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: sensor height = 0x%x (%d)\n", __func__, sval,sval);
//
//// WINWV_OB[3:0]   = 0x303A
//    addr = 0x303A;
//    imx183_read_reg(s_data, addr, &low);
//    sval = (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: WINWV_OB = 0x%x (%d)\n", __func__, sval,sval);
//
...
//    imx183_read_reg(s_data, addr, &low);
//    sval = (high << 8) | (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: X_OUT_SIZE = 0x%x (%d)\n", __func__, sval,sval);
//
...
//    imx183_read_reg(s_data, addr, &low);
//    sval = (high << 8) | (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: Y_OUT_SIZE = 0x%x (%d)\n", __func__, sval,sval);
//
...
//    imx183_read_reg(s_data, addr, &low);
//    sval = (high << 8) | (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: HMAX = 0x%x (%d)\n", __func__, sval,sval);
//
//// VMAX[17:0] H,M,L =  0x301A,0x3019,0x3018
//    addr = 0x301A;
//    imx183_read_reg(s_data, addr, &high);
//    addr = 0x3019;
//    imx183_read_reg(s_data, addr, &mid);
//    addr = 0x3018;
//    imx183_read_reg(s_data, addr, &low);
//    sval = (high << 16) | (mid << 8) | (low & 0xFF);
//    dev_err(dev, "%s(): IMX183: VMAX = 0x%x (%d)\n", __func__, sval,sval);
//
...
////    imx183_read_reg(s_data, addr, &high);
////    dev_err(dev, "%s(): IMX183: embedded data line control = 0x%x\n", __func__, (int)high);
//
////done:
//    return err;
//}
...
/****** imx183_start_streaming = Start streaming = 02.2020 **************/
static int imx183_start_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX183_START_STREAMING       1   /* DDD - imx183_start_streaming() - trace */
#define IMX183_START_STREAMING_SET_CTRLS   1   /* CCC - imx183_start_streaming() - set controls */
#define IMX183_START_STREAMING_DELAY       0 // 300 /* CCC - imx183_start_streaming() - delay in ms after start, 0=off */
//#define VC_EXT_TRIG_SET_EXP                1   /* CCC - imx183_start_streaming() - set exposure in ext. trigger code */
...
#if TRACE_IMX183_START_STREAMING
//    dev_err(dev, "%s():...\n", __func__);
#endif


// Set gain and exposure before streaming start
#if IMX183_START_STREAMING_SET_CTRLS
      imx183_set_gain(tc_dev, priv->digital_gain);
      imx183_set_exposure(tc_dev, priv->exposure_time);
      imx183_set_frame_rate(tc_dev, priv->frame_rate);
//      imx183_set_frame_length(tc_dev, priv->frame_length);
      mdelay(100);
#endif

#if TRACE_IMX183_START_STREAMING
//    dev_info(dev, "%s(): Set frame_length=%d\n", __func__, IMX183_DEFAULT_FRAME_LENGTH);
//    dev_info(dev, "%s(): Set gain=%d\n", __func__, IMX183_DEFAULT_GAIN);
#endif
...
//    mutex_lock(&priv->streaming_lock);
    err = imx183_write_table(priv, imx183_mode_table[IMX183_MODE_START_STREAM]);
    if (err) {
...
#if IMX183_START_STREAMING_DELAY
    dev_err(dev, "%s(): Delay after streaming start: %d ms\n", __func__, IMX183_START_STREAMING_DELAY);
    mdelay(IMX183_START_STREAMING_DELAY);    // ??? delay after start streaming
#endif
...
/****** imx183_stop_streaming = Stop streaming = 02.2020 ****************/
static int imx183_stop_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX183_STOP_STREAMING     1   /* DDD - imx183_stop_streaming() - trace */
//#define IMX183_STOP_STREAMING_ENB_RESET    0    /* CCC - imx183_stop_streaming() - enable VC MIPI reset */
...
#if TRACE_IMX183_STOP_STREAMING
//    dev_err(dev, "%s():\n", __func__);
#endif

    err = imx183_write_table(priv, imx183_mode_table[IMX183_MODE_STOP_STREAM]);
    if (err)
    {
//#if TRACE_IMX183_STOP_STREAMING
      dev_err(dev, "%s(): imx183_write_table() err=%d\n", __func__, err);
//#endif
//        mutex_unlock(&priv->streaming_lock);
        goto exit;
    }
...
//#if IMX183_STOP_STREAMING_ENB_RESET  // [[[
//        err = vc_mipi_reset(tc_dev, sensor_mode);
//        if(err)
//        {
...
static int imx183_set_mode(struct tegracam_device *tc_dev);

static struct camera_common_sensor_ops imx183_common_ops = {
    .numfrmfmts = ARRAY_SIZE(imx183_frmfmt),
    .frmfmt_table = imx183_frmfmt,
    .power_on = imx183_power_on,
    .power_off = imx183_power_off,
    .write_reg = imx183_write_reg,
    .read_reg = imx183_read_reg,
    .parse_dt = imx183_parse_dt,
    .power_get = imx183_power_get,
    .power_put = imx183_power_put,
    .set_mode = imx183_set_mode,
    .start_streaming = imx183_start_streaming,
    .stop_streaming = imx183_stop_streaming,
};

/****** imx183_set_mode = Set mode = 02.2020 ****************************/
static int imx183_set_mode(struct tegracam_device *tc_dev)
{

#define TRACE_IMX183_SET_MODE            1   /* DDD - imx183_set_mode() - trace */
#define IMX183_SET_MODE_DUMP_DT_PARAMS   0   /* DDD - imx183_set_mode() - dump DT params */
#define DUMP_SENSOR_MODE                 1   /* DDD - imx183_set_mode() - dump sensor mode */
#define DUMP_IMX183_REGS1                0   /* DDD - imx183_set_mode() - dump sensor regs aft set mode */

//#if !SKIP_IMX183_SET_MODE
    struct imx183 *priv = (struct imx183 *)tegracam_get_privdata(tc_dev);
//#endif
    struct camera_common_data *s_data = tc_dev->s_data;
    struct device *dev = tc_dev->dev;
...
#if IMX183_TRIG_FIX || IMX183_SET_MODE_DUMP_DT_PARAMS
    struct sensor_mode_properties *mode =  &s_data->sensor_props.sensor_modes[s_data->mode_prop_idx];
#endif
...
#if IMX183_TRIG_FIX   // trig mode fix
        tc_dev->s_data->numfmts = ARRAY_SIZE(imx183_trig_frmfmt);  // tc_dev->sensor_ops->numfrmfmts;
        tc_dev->s_data->frmfmt  = imx183_trig_frmfmt;             // tc_dev->sensor_ops->frmfmt_table;

        imx183_common_ops.numfrmfmts = ARRAY_SIZE(imx183_trig_frmfmt);
        imx183_common_ops.frmfmt_table = imx183_trig_frmfmt;

        mode->image_properties.height = IMX183_DY-2;
        s_data->fmt_height = IMX183_DY-2;
#endif
      }
#if IMX183_TRIG_FIX   // trig mode fix
      else    // free-run mode
      {
        tc_dev->s_data->numfmts = ARRAY_SIZE(imx183_frmfmt);  // tc_dev->sensor_ops->numfrmfmts;
        tc_dev->s_data->frmfmt  = imx183_frmfmt;              // tc_dev->sensor_ops->frmfmt_table;

        imx183_common_ops.numfrmfmts = ARRAY_SIZE(imx183_frmfmt);
        imx183_common_ops.frmfmt_table = imx183_frmfmt;

        mode->image_properties.height = IMX183_DY;
        s_data->fmt_height = IMX183_DY;
      }
#endif
      if(sensor_mode_id == 0)
...
//    if(s_data->fmt_width >= 1920 && s_data->fmt_height >= 1080)
    {
      priv->cam_mode = IMX183_MODE_5440X3648;
    }
//    else
//    {
...
//    if(priv->cam_mode != IMX183_MODE_5440X3648)
    {
      err = imx183_write_table(priv, imx183_mode_table[priv->cam_mode]);
      if (err)
      {
        dev_err(dev, "%s(): imx183_write_table() error=%d\n", __func__, err);
//        goto exit;
      }
      else
      {
#if TRACE_IMX183_SET_MODE
//        dev_err(dev, "%s(): imx183_write_table() OK, cam_mode=%d\n", __func__, priv->cam_mode);
#endif
      }
    }

#if IMX183_SET_MODE_DUMP_DT_PARAMS
{
...
/****** imx183_video_probe = Video probe = 02.2020 **********************/
static int imx183_video_probe(struct i2c_client *client)
{
...
/****** imx183_probe_vc_rom = Probe VC ROM = 02.2020 ********************/
static struct i2c_client * imx183_probe_vc_rom(struct i2c_adapter *adapter, u8 addr)
{
...
/****** imx183_board_setup = Board setup = 02.2020 **********************/
static int imx183_board_setup(struct imx183 *priv)
{

#define TRACE_IMX183_BOARD_SETUP   1   /* DDD - imx183_board_setup() - trace */
#define DUMP_CTL_REG_DATA          0   /* DDD - imx183_board_setup() - dump module control registers 0x100-0x108 (I2C addr=0x10) */
#define DUMP_HWD_DESC_ROM_DATA     0   /* DDD - imx183_board_setup() - dump Hardware Desriptor ROM data (I2C addr=0x10) */
#define DUMP_IMX183_REGS           0   /* DDD - imx183_board_setup() - dump IMX183 regs */
#define DUMP_V4L_PARAMS            1   /* DDD - imx183_board_setup() - dump V4L params */
#define DUMP_ROM_TABLE_REGS        0   /* DDD - imx183_board_setup() - dump ROM table regs */

#define MOD_ID                0x0183   /* CCC - imx183_board_setup() - module id., 0=off: don't check */
#define ERR_MOD_ID                 0   /* DDD - imx183_board_setup() - invalid module id test */
...
//    priv->rom = i2c_new_dummy(adapter,fpga_addr);
    priv->rom = imx183_probe_vc_rom(adapter,priv->fpga_addr);
...
//................ Check model:
//        if(priv->rom_table.mod_id == 0x0183)
//        {
//          dev_err(dev, "%s(): IMX183 sensor OK\n", __func__);
//        }
//        else
//        {
...
//          dev_err(&client->dev, "0x%04x: %02x\n", addr, reg_val[i]);
    ...
    ret = imx183_video_probe(client);
    if (ret < 0)
    {
      dev_err(dev, "%s(): imx183_video_probe() error=%d\n", __func__, ret);
      err = -EIO;
      goto done;
    }
...
/****** imx183_open = Open device = 02.2020 *****************************/
static int imx183_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
...
static const struct v4l2_subdev_internal_ops imx183_subdev_internal_ops = {
    .open = imx183_open,
};

static const struct of_device_id imx183_of_match[] = {
    { .compatible = "nvidia,imx183", },
    { },
};
MODULE_DEVICE_TABLE(of, imx183_of_match);

/****** imx183_probe = Probe = 02.2020 **********************************/
static int imx183_probe(struct i2c_client *client,
...
//    dev_info(dev, "%s(): Probing v4l2 sensor at addr 0x%0x\n", __func__, client->addr); // , __DATE__, __TIME__);
    ...
    priv = devm_kzalloc(dev,
                sizeof(struct imx183), GFP_KERNEL);
    if (!priv)
        return -ENOMEM;
    ...
    tc_dev->dev = dev;
    strncpy(tc_dev->name, "imx183", sizeof(tc_dev->name));
    tc_dev->dev_regmap_config = &imx183_regmap_config;
    tc_dev->sensor_ops = &imx183_common_ops;
    tc_dev->v4l2sd_internal_ops = &imx183_subdev_internal_ops;
    tc_dev->tcctrl_ops = &imx183_ctrl_ops;
    ...
    err = imx183_board_setup(priv);
    if (err) {
        dev_err(dev, "%s: imx183_board_setup() error=%d\n", __func__, err);
        return err;
    }
    dev_info(dev, "%s(): imx183_board_setup() OK\n", __func__);
    ...
    priv->digital_gain  = IMX183_DIGITAL_GAIN_DEFAULT;
    priv->exposure_time = IMX183_DIGITAL_EXPOSURE_DEFAULT;
    priv->frame_rate    = IMX183_FRAME_RATE_DEFAULT;
//    priv->frame_length = IMX183_DEFAULT_FRAME_LENGTH;
    if(ext_trig_mode >= 0)
...
//    priv->sen_clk = 72000000;     // clock-frequency: default=54Mhz imx183=72Mhz
    priv->sen_clk = IMX183_CLOCK_FREQUENCY;
...

{
    ...
    mode->control_properties.default_gain         = IMX183_DIGITAL_GAIN_DEFAULT;
    mode->control_properties.default_exp_time.val = IMX183_DIGITAL_EXPOSURE_DEFAULT;
}


//    set_sensor_model(imx183_sensor_model);
    set_sensor_model("imx183");
    dev_err(dev, "%s(): Detected imx183 sensor - %s/%s\n", __func__, __DATE__, __TIME__);

//    switch(priv->model)
//    {
//      case IMX183_MODEL_MONOCHROME:
//        priv->cam_mode = IMX183_MODE_5440X3648;
//        dev_err(dev, "%s(): Detected imx183 sensor\n", __func__); // , __DATE__, __TIME__);
//        break;
//
//      case IMX183_MODEL_COLOR:
//        priv->cam_mode = IMX183_MODE_5440X3648;
//        dev_err(dev, "%s(): Detected imx183c sensor\n", __func__); // , __DATE__, __TIME__);
//        break;
//
//      case IMX297_MODEL_MONOCHROME:
//        priv->cam_mode = IMX297_MODE_720X540;
...
//
//      default:
//        dev_err(dev, "%s(): Detected inknown imx183/297 sensor\n", __func__); // , __DATE__, __TIME__);
//        break;
//    }
...
/****** imx183_remove = Remove = 02.2020 ********************************/
static int imx183_remove(struct i2c_client *client)
{
    struct camera_common_data *s_data = to_camera_common_data(&client->dev);
    struct imx183 *priv = (struct imx183 *)s_data->priv;
...
static const struct i2c_device_id imx183_id[] = {
    { "imx183", 0 },
    { }
};

MODULE_DEVICE_TABLE(i2c, imx183_id);



/****** i2c_driver = I2C driver = 02.2020 *******************************/
static struct i2c_driver imx183_i2c_driver = {
    .driver = {
        .name = "imx183",
        .owner = THIS_MODULE,
        .of_match_table = of_match_ptr(imx183_of_match),
    },
    .probe = imx183_probe,
    .remove = imx183_remove,
    .id_table = imx183_id,
};
module_i2c_driver(imx183_i2c_driver);

MODULE_DESCRIPTION("Media Controller driver for IMX183");
