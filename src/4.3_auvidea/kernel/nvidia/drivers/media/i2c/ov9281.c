...
//      exposure = (exposure_time*1000 / 8721) * 16; // value in ns - 4 bit shift
      exposure = (exposure_time*1000 / 9100) * 16; // value in ns - 4 bit shift
...
//        int step = 4;
//
//        dev_info(&client->dev, "%s(): Dump Hardware Descriptor ROM data:\n", __func__);
//
//        addr = 0x1000;
//        while(addr < 0x10d0)
//        {
...
//
//          if(step == 4)
//            dev_info(&client->dev, "0x%04x: %02x %02x %02x %02x\n",addr,reg_buf[0],reg_buf[1],reg_buf[2],reg_buf[3]);
//          else
//            dev_info(&client->dev, "0x%04x: %02x %02x\n",addr,reg_buf[0],reg_buf[1]);
//          addr += step;
//        }
//
...
////        {
////          reg = reg_read(priv->rom, addr);
////          dev_info(&client->dev, "[IMX412]: imx412_probe(): addr=0x%04x reg=0x%02x\n",addr,reg);
////        }
//}
//#endif      /* ]]] */
...
//       }
//
//       dev_info(&client->dev, "%s(): VC Sensor MODE=%d PowerOn STATUS=0x%02x i=%d\n",__func__, sensor_mode,reg,i);
