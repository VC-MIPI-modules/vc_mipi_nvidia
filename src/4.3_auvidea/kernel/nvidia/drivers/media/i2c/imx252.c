...
#define VC_CODE             1   /* CCC - imx252.c - enable code for VC MIPI camera  */
#define IMX252_ENB_MUTEX    0   /* CCC - imx252.c - mutex lock/unlock */
#define IMX252_TRIG_MODE    0   /* CCC - imx252.c - force sensor ext. trigger mode */
#define IMX252_TRIG_FIX     0   /* CCC - imx252.c - fix trigger mode problem:                     */
                                /* in trigger mode the sensor produces frame height = IMX252_DY-2 */
                                /* switch between frame format tables:                            */
                                /*   imx252_frmfmt      : in free-run mode                        */
                                /*   imx252_trig_frmfmt : in trigger mode                         */

#define STOP_STREAMING_SENSOR_RESET   1   /* CCC - imx252_stop_streaming() - reset sensor before streaming stop */
...
#define IMX252_TABLE_WAIT_MS    0
#define IMX252_TABLE_END        1
...
#define IMX252_FRAME_RATE_DEFAULT   30000000   // 30 fps (*1000000)
...
enum {
    IMX252_MODE_2048X1536 = 0,
    IMX252_MODE_START_STREAM,
    IMX252_MODE_STOP_STREAM,
};

#define imx252_reg struct reg_8

static const imx252_reg imx252_start[] = {
    {0x7000, 0x01},         /* mode select streaming on */
    {IMX252_TABLE_END, 0x00}
};

static const imx252_reg imx252_stop[] = {
...
// #define IMX252_DY 1536 // TEGRA_VI_CSI_ERROR_STATUS=0x00000004 (Bits 3-0: h>,<, w>,<)
#define IMX252_DY 1535
// #define IMX252_DY 1534 // TEGRA_VI_CSI_ERROR_STATUS=0x00000008 (Bits 3-0: h>,<, w>,<)

//#if TEST_1408
//  #undef  IMX252_DX
//  #define IMX252_DX 1408
//#endif
//
//#if IMX252_TRIG_FIX // trig mode frame DY:
//  #define IMX252_DY 1078
//#else
//  #define IMX252_DY 1080
//#endif
//

static const imx252_reg imx252_mode_2048X1536[] = {
//#if 0
//#if TEST_1408
...
static const imx252_reg *imx252_mode_table[] = {
    [IMX252_MODE_2048X1536]    = imx252_mode_2048X1536,
    [IMX252_MODE_START_STREAM] = imx252_start,
    [IMX252_MODE_STOP_STREAM]  = imx252_stop,
};

static const int imx252_120fps[] = {
    120,
};

static const struct camera_common_frmfmt imx252_frmfmt[] = {
    { { IMX252_DX, IMX252_DY }, imx252_120fps, ARRAY_SIZE(imx252_120fps), 0,
      IMX252_MODE_2048X1536 },
};

//#if IMX252_TRIG_FIX   // [[[ - trig mode fix: does not work in free-run
  static const struct camera_common_frmfmt imx252_trig_frmfmt[] = {
    { { IMX252_DX, IMX252_DY-2 }, imx252_120fps, ARRAY_SIZE(imx252_120fps), 0,
      IMX252_MODE_2048X1536 },
};
//#endif  // ]]]
...
static const struct regmap_config imx252_regmap_config = {
    .reg_bits = 16,
    .val_bits = 8,
...
//               const struct imx252_reg table[])
//{
//    const struct imx252_reg *reg;
//    int ret;
//
//    for (reg = table; reg->addr != IMX252_TABLE_END; reg++) {
...
/****** imx252_read_reg = Read register = 11.2020 ***********************/
static int imx252_read_reg(struct camera_common_data *s_data, u16 addr, u8 *val)
{
    int err = 0;
    u32 reg_val = 0;
...
/****** imx252_write_reg = Write register = 11.2020 *********************/
static int imx252_write_reg(struct camera_common_data *s_data, u16 addr, u8 val)
{
    int err = 0;
...
/****** imx252_write_table = Write table = 11.2020 **********************/
static int imx252_write_table(struct imx252 *priv, const imx252_reg table[])
{
    return regmap_util_write_table_8(priv->s_data->regmap, table, NULL, 0,
        IMX252_TABLE_WAIT_MS, IMX252_TABLE_END);
}
...
//    u8  val;
//
//    err = imx252_read_reg(s_data, addr, &val);
//    if(err) return -1;
//
//    return (int)val;
//}
...
//{
//    return imx252_write_reg(s_data, addr, val);
//}


/****** imx252_gpio_set = GPIO set = 11.2020 ****************************/
static void imx252_gpio_set(struct camera_common_data *s_data,
...
/****** imx252_set_gain = Set gain = 11.2020 ****************************/
static int imx252_set_gain(struct tegracam_device *tc_dev, s64 val)
{

#define TRACE_IMX252_SET_GAIN         1   /* DDD - imx252_set_gain() - trace */
#define IMX252_SET_GAIN_STOP_STREAM   0   /* CCC - imx252_set_gain() - stop streaming before set gain */
//#define IMX252_SET_GAIN_GROUP_HOLD   0   /* CCC - imx252_set_gain() - enable group hold bef reg write */

//    struct camera_common_data *s_data = tc_dev->s_data;
    struct imx252 *priv = (struct imx252 *)tc_dev->priv;
...
//    s64 gain = val;
    ...
    if ( priv->digital_gain < IMX252_DIGITAL_GAIN_MIN)
       priv->digital_gain = IMX252_DIGITAL_GAIN_MIN;
    else if ( priv->digital_gain > IMX252_DIGITAL_GAIN_MAX)
       priv->digital_gain = IMX252_DIGITAL_GAIN_MAX;

//    gain = IMX252_DIGITAL_GAIN_DEFAULT; // VC: FIXED to 1.00 for now


//............. Stop streaming before gain change
#if IMX252_SET_GAIN_STOP_STREAM
    if(priv->streaming)
    {
      ret = imx252_write_table(priv, imx252_mode_table[IMX252_MODE_STOP_STREAM]);
      if(ret)
      {
        dev_err(dev, "%s(): imx252_write_table() err=%d\n", __func__, ret);
        return ret;
      }
    }
#endif

#if TRACE_IMX252_SET_GAIN
...
//............. Start streaming after gain change
    ...
    if(priv->streaming)
    {
      ret = imx252_write_table(priv, imx252_mode_table[IMX252_MODE_START_STREAM]);
      if(ret)
      {
        dev_err(dev, "%s(): imx252_write_table() err=%d\n", __func__, ret);
        return ret;
...
/****** imx_exposure_imx252 = IMX252 exposure = 11.2020 *****************/
static int imx_exposure_imx252(struct imx252 *priv, s32 expMin0, s32 expMin1, s32 expMax, s32 nrLines, s32 tOffset, s32 h1Period, s32 vMax)
{
//    struct i2c_client *client = v4l2_get_subdevdata(&priv->subdev);
    struct i2c_client *client = priv->i2c_client;
    int ret=0;
...
/****** imx252_set_exposure = Set exposure = 11.2020 ********************/
static int imx252_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
...
#define TRACE_IMX252_SET_EXPOSURE           1   /* DDD - imx252_set_exposure() - trace */
#define DUMP_EXPOSURE_PARAMS                0   /* DDD - imx252_set_exposure() - dump DT exposure params */
#define IMX252_SET_EXPOSURE_STOP_STREAM     0   /* CCC - imx252_set_exposure() - stop streaming before set exposure */

    struct device *dev = tc_dev->dev;
    struct imx252 *priv = tc_dev->priv;
//    struct imx252 *priv = (struct imx252 *)tc_dev->priv;
//    struct i2c_client *client = v4l2_get_subdevdata(&priv->subdev);
...
//    if(priv->exposure_time < IMX252_DIGITAL_EXPOSURE_MIN) priv->exposure_time = IMX252_DIGITAL_EXPOSURE_MIN;
//    if(priv->exposure_time > IMX252_DIGITAL_EXPOSURE_MAX) priv->exposure_time = IMX252_DIGITAL_EXPOSURE_MAX;

    if (priv->exposure_time < IMX252_DIGITAL_EXPOSURE_MIN)
        priv->exposure_time = IMX252_DIGITAL_EXPOSURE_MIN;

    if (priv->exposure_time > IMX252_DIGITAL_EXPOSURE_MAX)
        priv->exposure_time = IMX252_DIGITAL_EXPOSURE_MAX;
...
//#if IMX252_SET_EXPOSURE_STOP_STREAM
//    if(priv->streaming)
//    {
//      ret = imx252_write_table(priv, imx252_mode_table[IMX252_MODE_STOP_STREAM]);
//      if(ret)
...
/****** imx252_set_frame_rate = Set frame rate = 11.2020 ****************/
static int imx252_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{

#define TRACE_IMX252_SET_FRAME_RATE     0   /* DDD - imx252_set_frame_rate() - trace */
...
//
//#if TRACE_IMX252_SET_FRAME_RATE
//    dev_err(dev, "%s: frame_rate=%d: frame_length=%d\n", __func__, (int)val,frame_length);
//#endif
//
//    if(frame_length < IMX252_MIN_FRAME_LENGTH) frame_length = IMX252_MIN_FRAME_LENGTH;
//    if(frame_length > IMX252_MAX_FRAME_LENGTH) frame_length = IMX252_MAX_FRAME_LENGTH;
//
////      err = imx252_set_frame_length(tc_dev, frame_length);
////      if (err)
////         goto fail;
//
...
/****** imx252_set_group_hold = Set group hold = 11.2020 ****************/
static int imx252_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
    /* imx252 does not support group hold */
    return 0;
}
...
static struct tegracam_ctrl_ops imx252_ctrl_ops = {
    .numctrls = ARRAY_SIZE(ctrl_cid_list),
    .ctrl_cid_list = ctrl_cid_list,
    .set_gain = imx252_set_gain,
    .set_exposure = imx252_set_exposure,
    .set_frame_rate = imx252_set_frame_rate,  // imx252_set_frame_length,
    .set_group_hold = imx252_set_group_hold,

};

/****** imx252_power_on = Power on = 11.2020 ****************************/
static int imx252_power_on(struct camera_common_data *s_data)
{
#define TRACE_IMX252_POWER_ON   0   /* DDD - imx252_power_on() - trace */
...
//    }
    ...
    if (pw->avdd) {
        ...
        if (err)
            goto imx252_avdd_fail;
    }

    if (pw->iovdd) {
        err = regulator_enable(pw->iovdd);
        if (err)
            goto imx252_iovdd_fail;
    }
...
skip_power_seqn:
    ...
    if (gpio_is_valid(pw->pwdn_gpio))
        imx252_gpio_set(s_data, pw->pwdn_gpio, 1);
...
//    msleep(20);
    ...
    if (gpio_is_valid(pw->reset_gpio))
        imx252_gpio_set(s_data, pw->reset_gpio, 1);
...
/****** imx252_power_off = Power off = 11.2020 **************************/
static int imx252_power_off(struct camera_common_data *s_data)
{

#define TRACE_IMX252_POWER_OFF  0   /* DDD - imx252_power_off() - trace */
    ...
    usleep_range(21, 25);
    if (gpio_is_valid(pw->pwdn_gpio))
        imx252_gpio_set(s_data, pw->pwdn_gpio, 0);
    usleep_range(1, 2);
    if (gpio_is_valid(pw->reset_gpio))
        imx252_gpio_set(s_data, pw->reset_gpio, 0);
...
/****** imx252_power_put = Power put = 11.2020 **************************/
static int imx252_power_put(struct tegracam_device *tc_dev)
{
    struct camera_common_data *s_data = tc_dev->s_data;
    struct camera_common_power_rail *pw = s_data->power;
//    struct camera_common_pdata *pdata = s_data->pdata;
...
/****** imx252_power_get = Power get = 11.2020 **************************/
static int imx252_power_get(struct tegracam_device *tc_dev)
{

#define TRACE_IMX252_POWER_GET  0   /* DDD - imx252_power_get() - trace */
#define RESET_GPIO_ENB          0   /* CCC - imx252_power_get() - enable reset_gpio code */
...
/****** imx252_parse_dt = Parse DT = 11.2020 ****************************/
static struct camera_common_pdata *imx252_parse_dt(
    struct tegracam_device *tc_dev)
{

#define TRACE_IMX252_PARSE_DT   1   /* DDD - imx252_parse_dt() - trace */

// IMX219 : no parse clocks
//#define PARSE_CLOCKS        0   /* CCC - imx252_parse_dt() - parse clocks */
//#define PARSE_GPIOS         0   /* CCC - imx252_parse_dt() - parse GPIOss */

    struct device *dev = tc_dev->dev;
    struct device_node *node = dev->of_node;
    struct camera_common_pdata *board_priv_pdata;
//    const struct of_device_id *match;
//    struct imx252 *priv = (struct imx252 *)tegracam_get_privdata(tc_dev);
...
//    match = of_match_device(imx252_of_match, dev);
//    if (!match) {
...
/****** imx252_start_streaming = Start streaming = 11.2020 **************/
static int imx252_start_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX252_START_STREAMING       1   /* DDD - imx252_start_streaming() - trace */
#define IMX252_START_STREAMING_SET_CTRLS   1   /* CCC - imx252_start_streaming() - set controls */
#define IMX252_START_STREAMING_DELAY       0   // 300 /* CCC - imx252_start_streaming() - delay in ms after start, 0=off */
#define VC_EXT_TRIG_SET_EXP                1   /* CCC - imx252_start_streaming() - set exposure in ext. trigger code */
...
#if TRACE_IMX252_START_STREAMING
//    dev_err(dev, "%s():...\n", __func__);
#endif


// Set gain and exposure before streaming start
#if IMX252_START_STREAMING_SET_CTRLS
      imx252_set_gain(tc_dev, priv->digital_gain);
      imx252_set_exposure(tc_dev, priv->exposure_time);
      imx252_set_frame_rate(tc_dev, priv->frame_rate);
//      imx252_set_frame_length(tc_dev, priv->frame_length);
      mdelay(100);
#endif

#if TRACE_IMX252_START_STREAMING
//    dev_info(dev, "%s(): Set frame_length=%d\n", __func__, IMX252_DEFAULT_FRAME_LENGTH);
//    dev_info(dev, "%s(): Set gain=%d\n", __func__, IMX252_DEFAULT_GAIN);
#endif
...
//    mutex_lock(&priv->streaming_lock);
    err = imx252_write_table(priv, imx252_mode_table[IMX252_MODE_START_STREAM]);
    if (err) {
...
#if IMX252_START_STREAMING_DELAY
    dev_err(dev, "%s(): Delay after streaming start: %d ms\n", __func__, IMX252_START_STREAMING_DELAY);
    mdelay(IMX252_START_STREAMING_DELAY);    // ??? delay after start streaming
#endif
...
/****** imx252_stop_streaming = Stop streaming = 11.2020 ****************/
static int imx252_stop_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX252_STOP_STREAMING     1   /* DDD - imx252_stop_streaming() - trace */
//#define IMX252_STOP_STREAMING_ENB_RESET    0    /* CCC - imx252_stop_streaming() - enable VC MIPI reset */
...
#if TRACE_IMX252_STOP_STREAMING
//    dev_err(dev, "%s():\n", __func__);
#endif

    err = imx252_write_table(priv, imx252_mode_table[IMX252_MODE_STOP_STREAM]);
    if (err)
    {
//#if TRACE_IMX252_STOP_STREAMING
      dev_err(dev, "%s(): imx252_write_table() err=%d\n", __func__, err);
...
//#if IMX252_STOP_STREAMING_ENB_RESET  // [[[
...
static int imx252_set_mode(struct tegracam_device *tc_dev);

static struct camera_common_sensor_ops imx252_common_ops = {
    .numfrmfmts = ARRAY_SIZE(imx252_frmfmt),
    .frmfmt_table = imx252_frmfmt,
    .power_on = imx252_power_on,
    .power_off = imx252_power_off,
    .write_reg = imx252_write_reg,
    .read_reg = imx252_read_reg,
    .parse_dt = imx252_parse_dt,
    .power_get = imx252_power_get,
    .power_put = imx252_power_put,
    .set_mode = imx252_set_mode,
    .start_streaming = imx252_start_streaming,
    .stop_streaming = imx252_stop_streaming,
};

/****** imx252_set_mode = Set mode = 11.2020 ****************************/
static int imx252_set_mode(struct tegracam_device *tc_dev)
{

#define TRACE_IMX252_SET_MODE            1   /* DDD - imx252_set_mode() - trace */
#define IMX252_SET_MODE_DUMP_DT_PARAMS   0   /* DDD - imx252_set_mode() - dump DT params */
#define DUMP_SENSOR_MODE                 1   /* DDD - imx252_set_mode() - dump sensor mode */
#define DUMP_IMX252_REGS1                0   /* DDD - imx252_set_mode() - dump sensor regs aft set mode */

//#if !SKIP_IMX252_SET_MODE
    struct imx252 *priv = (struct imx252 *)tegracam_get_privdata(tc_dev);
//#endif
    struct camera_common_data *s_data = tc_dev->s_data;
    struct device *dev = tc_dev->dev;
...
#if IMX252_TRIG_FIX || IMX252_SET_MODE_DUMP_DT_PARAMS
    struct sensor_mode_properties *mode =  &s_data->sensor_props.sensor_modes[s_data->mode_prop_idx];
#endif
...
#if IMX252_TRIG_FIX   // trig mode fix
        tc_dev->s_data->numfmts = ARRAY_SIZE(imx252_trig_frmfmt);  // tc_dev->sensor_ops->numfrmfmts;
        tc_dev->s_data->frmfmt  = imx252_trig_frmfmt;             // tc_dev->sensor_ops->frmfmt_table;

        imx252_common_ops.numfrmfmts = ARRAY_SIZE(imx252_trig_frmfmt);
        imx252_common_ops.frmfmt_table = imx252_trig_frmfmt;

        mode->image_properties.height = IMX252_DY-2;
        s_data->fmt_height = IMX252_DY-2;
#endif
      }
#if IMX252_TRIG_FIX   // trig mode fix
      else    // free-run mode
      {
        tc_dev->s_data->numfmts = ARRAY_SIZE(imx252_frmfmt);  // tc_dev->sensor_ops->numfrmfmts;
        tc_dev->s_data->frmfmt  = imx252_frmfmt;              // tc_dev->sensor_ops->frmfmt_table;

        imx252_common_ops.numfrmfmts = ARRAY_SIZE(imx252_frmfmt);
        imx252_common_ops.frmfmt_table = imx252_frmfmt;

        mode->image_properties.height = IMX252_DY;
        s_data->fmt_height = IMX252_DY;
      }
#endif
      if(sensor_mode_id == 0)
...
//    if(s_data->fmt_width >= 1920 && s_data->fmt_height >= 1080)
    {
      priv->cam_mode = IMX252_MODE_2048X1536;
    }
//    else
//    {
...
//    if(priv->cam_mode != IMX252_MODE_2048X1536)
    {
      err = imx252_write_table(priv, imx252_mode_table[priv->cam_mode]);
      if (err)
      {
        dev_err(dev, "%s(): imx252_write_table() error=%d\n", __func__, err);
//        goto exit;
      }
      else
      {
#if TRACE_IMX252_SET_MODE
//        dev_err(dev, "%s(): imx252_write_table() OK, cam_mode=%d\n", __func__, priv->cam_mode);
#endif
      }
    }

#if IMX252_SET_MODE_DUMP_DT_PARAMS
{
//    struct sensor_mode_properties *mode =  &s_data->sensor_props.sensor_modes[s_data->mode_prop_idx];
...
/****** imx252_video_probe = Video probe = 11.2020 **********************/
static int imx252_video_probe(struct i2c_client *client)
{
...
/****** imx252_probe_vc_rom = Probe VC ROM = 11.2020 ********************/
static struct i2c_client * imx252_probe_vc_rom(struct i2c_adapter *adapter, u8 addr)
{
...
/****** imx252_board_setup = Board setup = 11.2020 **********************/
static int imx252_board_setup(struct imx252 *priv)
{

#define TRACE_IMX252_BOARD_SETUP   1   /* DDD - imx252_board_setup() - trace */
#define DUMP_CTL_REG_DATA          0   /* DDD - imx252_board_setup() - dump module control registers 0x100-0x108 (I2C addr=0x10) */
#define DUMP_HWD_DESC_ROM_DATA     0   /* DDD - imx252_board_setup() - dump Hardware Desriptor ROM data (I2C addr=0x10) */
#define DUMP_IMX252_REGS           0   /* DDD - imx252_board_setup() - dump IMX252 regs */
#define DUMP_V4L_PARAMS            1   /* DDD - imx252_board_setup() - dump V4L params */
#define DUMP_ROM_TABLE_REGS        0   /* DDD - imx252_board_setup() - dump ROM table regs */

//#define MOD_ID                0x0252   /* CCC - imx252_board_setup() - module id., 0=off: don't check */
//#define ERR_MOD_ID                 0   /* DDD - imx252_board_setup() - invalid module id test */
...
//    priv->rom = i2c_new_dummy(adapter,fpga_addr);
    priv->rom = imx252_probe_vc_rom(adapter,priv->fpga_addr);
...
//................ Check model:
        ...
        // IMX252
        if(priv->rom_table.mod_id == 0x0252)
        {
          if(color)
          {
            priv->model = IMX252_MODEL_COLOR;
          }
          else
          {
            priv->model = IMX252_MODEL_MONOCHROME;
          }
        }
...
////        if(priv->rom_table.mod_id == 0x0252)
////        {
////          dev_err(dev, "%s(): IMX252 sensor OK\n", __func__);
////        }
////        else
////        {
...
//[    2.195918] imx252 7-001a: imx252_board_setup(): Module controller registers (0x10):
//[    2.208811] imx252 7-001a: 0x100-0x103: 00 80 01 08
//[    2.213749] imx252 7-001a: 0x104-0x108: 10 1a 00 00 00
//[    2.218978] imx252 7-001a: 0x109-0x10C: 10 27 00 00
//[    2.223926] imx252 7-001a: 0x10D-0x110: 3e ec 07 00
    ...
    ret = imx252_video_probe(client);
    if (ret < 0)
    {
      dev_err(dev, "%s(): imx252_video_probe() error=%d\n", __func__, ret);
      err = -EIO;
      goto done;
    }
...
/****** imx252_open = Open device = 11.2020 *****************************/
static int imx252_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
...
static const struct v4l2_subdev_internal_ops imx252_subdev_internal_ops = {
    .open = imx252_open,
};

static const struct of_device_id imx252_of_match[] = {
    { .compatible = "nvidia,imx252", },
    { },
};
MODULE_DEVICE_TABLE(of, imx252_of_match);

/****** imx252_probe = Probe = 11.2020 **********************************/
static int imx252_probe(struct i2c_client *client,
...
//    dev_info(dev, "%s(): Probing v4l2 sensor at addr 0x%0x\n", __func__, client->addr); // , __DATE__, __TIME__);
    ...
    priv = devm_kzalloc(dev,
                sizeof(struct imx252), GFP_KERNEL);
    if (!priv)
        return -ENOMEM;
    ...
    tc_dev->dev = dev;
    strncpy(tc_dev->name, "imx252", sizeof(tc_dev->name));
    tc_dev->dev_regmap_config = &imx252_regmap_config;
    tc_dev->sensor_ops = &imx252_common_ops;
    tc_dev->v4l2sd_internal_ops = &imx252_subdev_internal_ops;
    tc_dev->tcctrl_ops = &imx252_ctrl_ops;
    ...
    err = imx252_board_setup(priv);
    if (err) {
        dev_err(dev, "%s: imx252_board_setup() error=%d\n", __func__, err);
        return err;
    }
    dev_info(dev, "%s(): imx252_board_setup() OK\n", __func__);
    ...
    priv->digital_gain  = IMX252_DIGITAL_GAIN_DEFAULT;
    priv->exposure_time = IMX252_DIGITAL_EXPOSURE_DEFAULT;
    priv->frame_rate    = IMX252_FRAME_RATE_DEFAULT;
//    priv->frame_length = IMX252_DEFAULT_FRAME_LENGTH;
    if(ext_trig_mode >= 0)
...
//    priv->sen_clk = 13000000;       // clock-frequency: imx252=74Mhz
...

{
    ...
    mode->control_properties.default_gain         = IMX252_DIGITAL_GAIN_DEFAULT;
    mode->control_properties.default_exp_time.val = IMX252_DIGITAL_EXPOSURE_DEFAULT;
}


//    set_sensor_model(imx252_sensor_model);
    set_sensor_model("imx252");

//    dev_err(dev, "%s(): Detected imx252 sensor - %s/%s\n", __func__, __DATE__, __TIME__);

    switch(priv->model)
    {
      case IMX252_MODEL_MONOCHROME:
        priv->cam_mode = IMX252_MODE_2048X1536;
        dev_err(dev, "%s(): Detected imx252 sensor\n", __func__); // , __DATE__, __TIME__);
        break;

      case IMX252_MODEL_COLOR:
        priv->cam_mode = IMX252_MODE_2048X1536;
        dev_err(dev, "%s(): Detected imx252c sensor\n", __func__); // , __DATE__, __TIME__);
        break;
...
/****** imx252_remove = Remove = 11.2020 ********************************/
static int imx252_remove(struct i2c_client *client)
{
    struct camera_common_data *s_data = to_camera_common_data(&client->dev);
    struct imx252 *priv = (struct imx252 *)s_data->priv;
...
static const struct i2c_device_id imx252_id[] = {
    { "imx252", 0 },
    { }
};

MODULE_DEVICE_TABLE(i2c, imx252_id);



/****** i2c_driver = I2C driver = o9.2020 *******************************/
static struct i2c_driver imx252_i2c_driver = {
    .driver = {
        .name = "imx252",
        .owner = THIS_MODULE,
        .of_match_table = of_match_ptr(imx252_of_match),
    },
    .probe = imx252_probe,
    .remove = imx252_remove,
    .id_table = imx252_id,
};
module_i2c_driver(imx252_i2c_driver);

MODULE_VERSION("1.04");
MODULE_DESCRIPTION("Media Controller driver for IMX252");
