...
#define VC_MIPI     1   /* CCC - vi2_fops.c - VC MIPI code (debugging messages) */
...
/****** vi2_channel_setup_queue = Setup queue = 05.2020 *****************/
static int vi2_channel_setup_queue(struct tegra_channel *chan,
    unsigned int *nbuffers)
{

#define TRACE_VI2_CHANNEL_SETUP_QUEUE   0   /* DDD - vi2_channel_setup_queue - trace */

    /* Make sure minimum number of buffers are passed */
    if (*nbuffers < (QUEUED_BUFFERS - 1))
        *nbuffers = QUEUED_BUFFERS - 1;

#if TRACE_VI2_CHANNEL_SETUP_QUEUE
    dev_err(&chan->video->dev, "vi2_fops: %s(): nbuffers=%d\n", __func__, *nbuffers);
#endif
...
/****** tegra_channel_capture_setup = Tegra channel capture setup = 04.2020 ****/
static int tegra_channel_capture_setup(struct tegra_channel *chan)
{

#define TRACE_TEGRA_CHANNEL_CAPTURE_SETUP       0  /* DDD - tegra_channel_capture_setup - trace */
#define TEGRA_CHANNEL_CAPTURE_SETUP_SET_MASK    0   /* CCC - tegra_channel_capture_setup - set interrupt mask */
...
#if TRACE_TEGRA_CHANNEL_CAPTURE_SETUP
    dev_err(&chan->video->dev, "%s(): valid_ports=%d: width,height=%d,%d word_count=%d\n", __func__, chan->valid_ports, width, height, word_count);
#endif
...
#if VC_MIPI
  #if TEGRA_CHANNEL_CAPTURE_SETUP_SET_MASK
// Bits 3-0: h>,<, w>,<
// Disable frame height long,short error: mask = 0x0c
{
        int val;
...
/****** tegra_channel_ec_init = Tegra channel EC init = 04.2020 *********/
static void tegra_channel_ec_init(struct tegra_channel *chan)
{

#define SET_TEGRA_CHANNEL_EC_INIT_TIMEOUT 0 // 500    /* DDD - tegra_channel_ec_init - set test timeout in ms, 0=off*/
     ...
     */
#if SET_TEGRA_CHANNEL_EC_INIT_TIMEOUT
    chan->timeout = msecs_to_jiffies(SET_TEGRA_CHANNEL_EC_INIT_TIMEOUT);
#else
...
/****** tegra_channel_clear_singleshot = Clear single shot = 04.2020 ***/
static void tegra_channel_clear_singleshot(struct tegra_channel *chan,
                        int index)
{

#define TRACE_TEGRA_CHANNEL_CLEAR_SINGLESHOT    0   /* +++ DDD - tegra_channel_clear_singleshot - trace */

#if TRACE_TEGRA_CHANNEL_CLEAR_SINGLESHOT
...
/****** tegra_channel_vi_csi_recover = VI CSI recover = 04.2020 ********/
static void tegra_channel_vi_csi_recover(struct tegra_channel *chan)
{

#define TRACE_TEGRA_CHANNEL_VI_CSI_RECOVER  0   /* +++ DDD - tegra_channel_vi_csi_recover - trace */
...
#if TRACE_TEGRA_CHANNEL_VI_CSI_RECOVER
    dev_err(&chan->video->dev, "%s(): start...\n", __func__);
#endif
...
#define TRACE_TEGRA_CHANNEL_CAPTURE_ERROR   0   /* +++ DDD - tegra_channel_capture_error - trace */
...
#if TRACE_TEGRA_CHANNEL_CAPTURE_ERROR
        dev_err(&chan->video->dev, "%s(): i=%d: TEGRA_VI_CSI_ERROR_STATUS=0x%08x\n", __func__, index, val);
//        dev_err(&chan->video->dev, "%s(): Bits 3-0: Frame dy >,<, dx >,< than expected\n", __func__);
#else
...
/****** tegra_channel_ec_recover = Tegra chan EC recover = 04.2020 ******/
static void tegra_channel_ec_recover(struct tegra_channel *chan)
{

#define TRACE_TEGRA_CHANNEL_EC_RECOVER      0   /* DDD - tegra_channel_ec_recover - trace */

#if TRACE_TEGRA_CHANNEL_EC_RECOVER
    dev_err(&chan->video->dev, "%s(): ...\n", __func__);
#endif
...
#define TEGRA_CHANNEL_ERROR_STATUS_DIS_ERROR    0   /* DDD - tegra_channel_error_status - disable error */
...
// ???
#if TEGRA_CHANNEL_ERROR_STATUS_DIS_ERROR
          val = 0;
#endif
          dev_err(chan->vi->dev, "%s(): i=%d: TEGRA_VI_CSI_ERROR_STATUS=0x%08x (Bits 3-0: h>,<, w>,<)\n", __func__, index, val);
//          dev_err(chan->vi->dev, "%s(): Bits 3-0: Frame dy >,<, dx >,< than expected\n", __func__);
        }
...
/****** tegra_channel_capture_frame_single_thread = Capture single thread = 04.2020 ***/
static int tegra_channel_capture_frame_single_thread(
            struct tegra_channel *chan,
            struct tegra_channel_buffer *buf)
{

#define TRACE_TEGRA_CHANNEL_CAPTURE_FRAME_SINGLE_THREAD         0   /* DDD - tegra_channel_capture_frame_single_thread - trace */
#define TRACE_TEGRA_CHANNEL_CAPTURE_FRAME_SINGLE_THREAD_RECOVER 0   /* +++ DDD - tegra_channel_capture_frame_single_thread - trace */
...
#if TRACE_TEGRA_CHANNEL_CAPTURE_FRAME_SINGLE_THREAD
    dev_err(&chan->video->dev, "=====> %s(): Start, valid_ports=%d\n", __func__, valid_ports);
#endif
...
#if TRACE_TEGRA_CHANNEL_CAPTURE_FRAME_SINGLE_THREAD
    dev_err(&chan->video->dev, "=====> %s(): After register init\n", __func__);
#endif
...
#if TRACE_TEGRA_CHANNEL_CAPTURE_FRAME_SINGLE_THREAD
    dev_err(&chan->video->dev, "=====> %s(): After 1st start\n", __func__);
#endif
    ...
    for (index = 0; index < valid_ports; index++) {
        ...
        if (err) {
            dev_err(&chan->video->dev,
                "%s: frame start syncpt timeout! i=%d: Before tegra_channel_ec_recover()...\n", __func__, index);
            state = VB2_BUF_STATE_REQUEUEING;
            /* perform error recovery for timeout */
            tegra_channel_ec_recover(chan);
#if TRACE_TEGRA_CHANNEL_CAPTURE_FRAME_SINGLE_THREAD_RECOVER
           dev_err(&chan->video->dev, "%s: After tegra_channel_ec_recover()\n", __func__);
#endif
...
/****** tegra_channel_capture_frame_multi_thread = Capture multi thread = 04.2020 ***/
...
/****** tegra_channel_capture_frame = Capture frame = 04.2020 ***********/
static int tegra_channel_capture_frame(struct tegra_channel *chan,
                    struct tegra_channel_buffer *buf)
{

#define TRACE_TEGRA_CHANNEL_CAPTURE_FRAME   0   /* DDD - tegra_channel_capture_frame - trace */

    int ret = 0;


#if TRACE_TEGRA_CHANNEL_CAPTURE_FRAME
    dev_err(&chan->video->dev, "\n");
    dev_err(&chan->video->dev, "----- %s: Begin: chan->low_latency=%d\n", __func__, chan->low_latency);
#endif
...
#if TRACE_TEGRA_CHANNEL_CAPTURE_FRAME
    dev_err(&chan->video->dev, "----- %s: Exit: ret=%d\n", __func__, ret);
#endif

    return ret;
}

/****** tegra_channel_release_frame = Release frame = 04.2020 ********/
static void tegra_channel_release_frame(struct tegra_channel *chan,
                struct tegra_channel_buffer *buf)
{

#define TRACE_TEGRA_CHANNEL_RELEASE_FRAME   0   /* +++ DDD - tegra_channel_release_frame - trace */
...
#if TRACE_TEGRA_CHANNEL_RELEASE_FRAME
    dev_err(&chan->video->dev, "%s: ....\n", __func__);
#endif
...
/****** tegra_channel_capture_done = Capture done = 04.2020 ************/
static void tegra_channel_capture_done(struct tegra_channel *chan)
{

#define TRACE_TEGRA_CHANNEL_CAPTURE_DONE    0   /* +++ DDD - tegra_channel_capture_done - trace */
...
#if TRACE_TEGRA_CHANNEL_CAPTURE_DONE
    dev_err(&chan->video->dev, "%s: ...\n",  __func__);
...
/****** tegra_channel_kthread_capture_start = Kthread capture start = 04.2020 ****/
static int tegra_channel_kthread_capture_start(void *data)
{

#define TRACE_TEGRA_CHANNEL_KTHREAD_CAPTURE_START   0   /* DDD - tegra_channel_kthread_capture_start - trace */

    struct tegra_channel *chan = data;
    struct tegra_channel_buffer *buf;
    int err = 0;

#if TRACE_TEGRA_CHANNEL_KTHREAD_CAPTURE_START
    int ipass = 0;
    dev_err(&chan->video->dev, "%s: Start ...\n",  __func__);
#endif
...
#if TRACE_TEGRA_CHANNEL_KTHREAD_CAPTURE_START
        dev_err(&chan->video->dev, "-----------------------------------------------------------\n");
        dev_err(&chan->video->dev, "%s: ipass=%d\n",  __func__, ipass++);
        dev_err(&chan->video->dev, "-----------------------------------------------------------\n");
#endif
...
/****** vi2_channel_start_streaming = Start streaming = 04.2020 ******/
static int vi2_channel_start_streaming(struct vb2_queue *vq, u32 count)
{

#define TRACE_VI2_CHANNEL_START_STREAMING   0   /* +++DDD - vi2_channel_start_streaming - trace */
...
#if TRACE_VI2_CHANNEL_START_STREAMING
    dev_err(&chan->video->dev, "%s(): ...\n", __func__);
#endif
