...
#define VC_MIPI     1   /* CCC - vi2_fops.c - VC MIPI code */
...
#define TRACE_TEGRA_CSI_STATUS      0   /* DDD - tegra_csi_status() - trace */
#define TEGRA_CSI_STATUS_DEV_ERR    1   /* DDD - tegra_csi_status() - use dev_err */
...
#if TRACE_TEGRA_CSI_STATUS
    DEV_DBG(chan->csi->dev,"%s(): chan->numports=%d\n", __func__, chan->numports);
#endif
...
/****** csi2_start_streaming    *****************************************/
static int csi2_start_streaming(struct tegra_csi_channel *chan, int port_idx)
{

#define TRACE_CSI2_START_STREAMING  0   /* DDD - csi2_start_streaming - trace */
...
#if TRACE_CSI2_START_STREAMING
    dev_err(csi->dev, "%s: csi_port=%d, csi_lanes=%d\n", __func__,  csi_port, csi_lanes);
#endif
...
//    pp_write(port, TEGRA_CSI_PIXEL_PARSER_INTERRUPT_MASK, 0x0c);
//#else
...
#define TRACE_CSI2_STOP_STREAMING   0   /* DDD - csi2_stop_streaming - trace */

    struct tegra_csi_port *port = &chan->ports[port_idx];
    unsigned int val, csi_port;

#if TRACE_CSI2_STOP_STREAMING
    struct tegra_csi_device *csi = chan->csi;
    dev_err(csi->dev, "%s: csi_port=%d\n", __func__,  port->csi_port);
#endif
...
#define TRACE_CSI2_MIPI_CAL  0  /* DDD - csi2_mipi_cal - trace */
...
#if TRACE_CSI2_MIPI_CAL
//    struct tegra_csi_device *csi = chan->csi;
    dev_err(csi->dev, "%s: chan->numports=%d\n", __func__,  chan->numports);
#endif
...
#if TRACE_CSI2_MIPI_CAL
        dev_err(csi->dev, "%s: Calibrate csi port %d\n", __func__,  csi_port);
#endif
