...
#define VC_CODE             1   /* CCC - imx250.c - enable code for VC MIPI camera  */
#define IMX250_ENB_MUTEX    0   /* CCC - imx250.c - mutex lock/unlock */
#define IMX250_TRIG_MODE    0   /* CCC - imx250.c - force sensor ext. trigger mode */
#define IMX250_TRIG_FIX     0   /* CCC - imx250.c - fix trigger mode problem:                     */
                                /* in trigger mode the sensor produces frame height = IMX250_DY-2 */
                                /* switch between frame format tables:                            */
                                /*   imx250_frmfmt      : in free-run mode                        */
                                /*   imx250_trig_frmfmt : in trigger mode                         */

#define STOP_STREAMING_SENSOR_RESET   1   /* CCC - imx250_stop_streaming() - reset sensor before streaming stop */
...
#define IMX250_TABLE_WAIT_MS    0
#define IMX250_TABLE_END        1
...
#define IMX250_FRAME_RATE_DEFAULT   30000000   // 30 fps (*1000000)
...
enum {
    IMX250_MODE_2432X2048 = 0,
    IMX250_MODE_START_STREAM,
    IMX250_MODE_STOP_STREAM,
};

#define imx250_reg struct reg_8

static const imx250_reg imx250_start[] = {
    {0x7000, 0x01},         /* mode select streaming on */
    {IMX250_TABLE_END, 0x00}
};

static const imx250_reg imx250_stop[] = {
...
//#if TEST_1408
//  #undef  IMX250_DX
//  #define IMX250_DX 1408
//#endif
//
//#if IMX250_TRIG_FIX // trig mode frame DY:
//  #define IMX250_DY 1078
//#else
//  #define IMX250_DY 1080
//#endif
//

static const imx250_reg imx250_mode_2432X2048[] = {
//#if 0
//#if TEST_1408
...
static const imx250_reg *imx250_mode_table[] = {
    [IMX250_MODE_2432X2048]    = imx250_mode_2432X2048,
    [IMX250_MODE_START_STREAM] = imx250_start,
    [IMX250_MODE_STOP_STREAM]  = imx250_stop,
};

static const int imx250_120fps[] = {
    60,
};

static const struct camera_common_frmfmt imx250_frmfmt[] = {
    { { IMX250_DX, IMX250_DY }, imx250_120fps, ARRAY_SIZE(imx250_120fps), 0,
      IMX250_MODE_2432X2048 },
};

//#if IMX250_TRIG_FIX   // [[[ - trig mode fix: does not work in free-run
  static const struct camera_common_frmfmt imx250_trig_frmfmt[] = {
    { { IMX250_DX, IMX250_DY-2 }, imx250_120fps, ARRAY_SIZE(imx250_120fps), 0,
      IMX250_MODE_2432X2048 },
};
//#endif  // ]]]
...
static const struct regmap_config imx250_regmap_config = {
    .reg_bits = 16,
    .val_bits = 8,
...
///****** reg_write_table = Write reg table = 11.2020 *******************/
//static int reg_write_table(struct i2c_client *client,
//               const struct imx250_reg table[])
//{
//    const struct imx250_reg *reg;
//    int ret;
//
//    for (reg = table; reg->addr != IMX250_TABLE_END; reg++) {
...
/****** imx250_read_reg = Read register = 11.2020 ***********************/
static int imx250_read_reg(struct camera_common_data *s_data, u16 addr, u8 *val)
{
    int err = 0;
    u32 reg_val = 0;
...
/****** imx250_write_reg = Write register = 11.2020 *********************/
static int imx250_write_reg(struct camera_common_data *s_data, u16 addr, u8 val)
{
    int err = 0;
...
/****** imx250_write_table = Write table = 11.2020 **********************/
static int imx250_write_table(struct imx250 *priv, const imx250_reg table[])
{
    return regmap_util_write_table_8(priv->s_data->regmap, table, NULL, 0,
        IMX250_TABLE_WAIT_MS, IMX250_TABLE_END);
}
...
///****** vc_mipi_common_reg_read = Read reg common = 11.2020 *************/
...
//    u8  val;
//
//    err = imx250_read_reg(s_data, addr, &val);
//    if(err) return -1;
//
//    return (int)val;
//}
//
///****** vc_mipi_common_reg_write = Write reg common = 11.2020 ***********/
//static int vc_mipi_common_reg_write(struct camera_common_data *s_data, u16 addr, u8 val)
//{
//    return imx250_write_reg(s_data, addr, val);
//}


/****** imx250_gpio_set = GPIO set = 11.2020 ****************************/
static void imx250_gpio_set(struct camera_common_data *s_data,
...
/****** imx250_set_gain = Set gain = 11.2020 ****************************/
static int imx250_set_gain(struct tegracam_device *tc_dev, s64 val)
{

#define TRACE_IMX250_SET_GAIN         1   /* DDD - imx250_set_gain() - trace */
#define IMX250_SET_GAIN_STOP_STREAM   0   /* CCC - imx250_set_gain() - stop streaming before set gain */
//#define IMX250_SET_GAIN_GROUP_HOLD   0   /* CCC - imx250_set_gain() - enable group hold bef reg write */

//    struct camera_common_data *s_data = tc_dev->s_data;
    struct imx250 *priv = (struct imx250 *)tc_dev->priv;
...
//    s64 gain = val;
    ...
    if ( priv->digital_gain < IMX250_DIGITAL_GAIN_MIN)
       priv->digital_gain = IMX250_DIGITAL_GAIN_MIN;
    else if ( priv->digital_gain > IMX250_DIGITAL_GAIN_MAX)
       priv->digital_gain = IMX250_DIGITAL_GAIN_MAX;

//    gain = IMX250_DIGITAL_GAIN_DEFAULT; // VC: FIXED to 1.00 for now


//............. Stop streaming before gain change
#if IMX250_SET_GAIN_STOP_STREAM
    if(priv->streaming)
    {
      ret = imx250_write_table(priv, imx250_mode_table[IMX250_MODE_STOP_STREAM]);
      if(ret)
      {
        dev_err(dev, "%s(): imx250_write_table() err=%d\n", __func__, ret);
        return ret;
      }
    }
#endif

#if TRACE_IMX250_SET_GAIN
...
//............. Start streaming after gain change
    ...
    if(priv->streaming)
    {
      ret = imx250_write_table(priv, imx250_mode_table[IMX250_MODE_START_STREAM]);
      if(ret)
      {
        dev_err(dev, "%s(): imx250_write_table() err=%d\n", __func__, ret);
        return ret;
...
// NumberOfLines=3694
//
#define H1PERIOD_250 327680 // (U32)(20.000 * 16384.0)
#define NRLINES_250  (3694)
#define TOFFSET_250  47563  // (U32)(2.903 * 16384.0)
#define VMAX_250     3079
#define EXPOSURE_TIME_MIN_250  160
#define EXPOSURE_TIME_MIN2_250 74480
#define EXPOSURE_TIME_MAX_250  10000000

#define imx_exposure_250(priv) imx_exposure_imx250(priv, EXPOSURE_TIME_MIN_250, EXPOSURE_TIME_MIN2_250, EXPOSURE_TIME_MAX_250, NRLINES_250 ,TOFFSET_250, H1PERIOD_250, VMAX_250)

/****** imx_exposure_imx250 = IMX250 exposure = 11.2020 *****************/
static int imx_exposure_imx250(struct imx250 *priv, s32 expMin0, s32 expMin1, s32 expMax, s32 nrLines, s32 tOffset, s32 h1Period, s32 vMax)
{
//    struct i2c_client *client = v4l2_get_subdevdata(&priv->subdev);
    struct i2c_client *client = priv->i2c_client;
    int ret=0;
...
/****** imx250_set_exposure = Set exposure = 11.2020 ********************/
static int imx250_set_exposure(struct tegracam_device *tc_dev, s64 val)
{

#define TRACE_IMX250_SET_EXPOSURE           1   /* DDD - imx250_set_exposure() - trace */
#define DUMP_EXPOSURE_PARAMS                0   /* DDD - imx250_set_exposure() - dump DT exposure params */
#define IMX250_SET_EXPOSURE_STOP_STREAM     0   /* CCC - imx250_set_exposure() - stop streaming before set exposure */

    struct device *dev = tc_dev->dev;
    struct imx250 *priv = tc_dev->priv;
//    struct imx250 *priv = (struct imx250 *)tc_dev->priv;
//    struct i2c_client *client = v4l2_get_subdevdata(&priv->subdev);
...
//    if(priv->exposure_time < IMX250_DIGITAL_EXPOSURE_MIN) priv->exposure_time = IMX250_DIGITAL_EXPOSURE_MIN;
//    if(priv->exposure_time > IMX250_DIGITAL_EXPOSURE_MAX) priv->exposure_time = IMX250_DIGITAL_EXPOSURE_MAX;

    if (priv->exposure_time < IMX250_DIGITAL_EXPOSURE_MIN)
        priv->exposure_time = IMX250_DIGITAL_EXPOSURE_MIN;

    if (priv->exposure_time > IMX250_DIGITAL_EXPOSURE_MAX)
        priv->exposure_time = IMX250_DIGITAL_EXPOSURE_MAX;
...
//#if IMX250_SET_EXPOSURE_STOP_STREAM
//    if(priv->streaming)
//    {
//      ret = imx250_write_table(priv, imx250_mode_table[IMX250_MODE_STOP_STREAM]);
//      if(ret)
...
/*............. Set IMX250 exposure */
      ...
      if(ret)
      {
          dev_err(dev, "%s(): imx_exposure_250() err=%d\n", __func__, ret);
          return ret;
      }
...
/****** imx250_set_frame_rate = Set frame rate = 11.2020 ****************/
static int imx250_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{

#define TRACE_IMX250_SET_FRAME_RATE     0   /* DDD - imx250_set_frame_rate() - trace */
...
//
//#if TRACE_IMX250_SET_FRAME_RATE
//    dev_err(dev, "%s: frame_rate=%d: frame_length=%d\n", __func__, (int)val,frame_length);
//#endif
//
//    if(frame_length < IMX250_MIN_FRAME_LENGTH) frame_length = IMX250_MIN_FRAME_LENGTH;
//    if(frame_length > IMX250_MAX_FRAME_LENGTH) frame_length = IMX250_MAX_FRAME_LENGTH;
//
////      err = imx250_set_frame_length(tc_dev, frame_length);
////      if (err)
////         goto fail;
//
...
/****** imx250_set_group_hold = Set group hold = 11.2020 ****************/
static int imx250_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
    /* imx250 does not support group hold */
    return 0;
}
...
static struct tegracam_ctrl_ops imx250_ctrl_ops = {
    .numctrls = ARRAY_SIZE(ctrl_cid_list),
    .ctrl_cid_list = ctrl_cid_list,
    .set_gain = imx250_set_gain,
    .set_exposure = imx250_set_exposure,
    .set_frame_rate = imx250_set_frame_rate,  // imx250_set_frame_length,
    .set_group_hold = imx250_set_group_hold,

};

/****** imx250_power_on = Power on = 11.2020 ****************************/
static int imx250_power_on(struct camera_common_data *s_data)
{
#define TRACE_IMX250_POWER_ON   0   /* DDD - imx250_power_on() - trace */
...
//    }
    ...
    if (pw->avdd) {
        ...
        if (err)
            goto imx250_avdd_fail;
    }

    if (pw->iovdd) {
        err = regulator_enable(pw->iovdd);
        if (err)
            goto imx250_iovdd_fail;
    }
...
skip_power_seqn:
    ...
    if (gpio_is_valid(pw->pwdn_gpio))
        imx250_gpio_set(s_data, pw->pwdn_gpio, 1);
...
//    msleep(20);
    ...
    if (gpio_is_valid(pw->reset_gpio))
        imx250_gpio_set(s_data, pw->reset_gpio, 1);
...
/****** imx250_power_off = Power off = 11.2020 **************************/
static int imx250_power_off(struct camera_common_data *s_data)
{

#define TRACE_IMX250_POWER_OFF  0   /* DDD - imx250_power_off() - trace */
    ...
    usleep_range(21, 25);
    if (gpio_is_valid(pw->pwdn_gpio))
        imx250_gpio_set(s_data, pw->pwdn_gpio, 0);
    usleep_range(1, 2);
    if (gpio_is_valid(pw->reset_gpio))
        imx250_gpio_set(s_data, pw->reset_gpio, 0);
...
/****** imx250_power_put = Power put = 11.2020 **************************/
static int imx250_power_put(struct tegracam_device *tc_dev)
{
    struct camera_common_data *s_data = tc_dev->s_data;
    struct camera_common_power_rail *pw = s_data->power;
//    struct camera_common_pdata *pdata = s_data->pdata;
...
/****** imx250_power_get = Power get = 11.2020 **************************/
static int imx250_power_get(struct tegracam_device *tc_dev)
{

#define TRACE_IMX250_POWER_GET  0   /* DDD - imx250_power_get() - trace */
#define RESET_GPIO_ENB          0   /* CCC - imx250_power_get() - enable reset_gpio code */
...
/****** read_property_u32 = Read U32 property = 11.2020 *****************/
...
/****** imx250_parse_dt = Parse DT = 11.2020 ****************************/
static struct camera_common_pdata *imx250_parse_dt(
    struct tegracam_device *tc_dev)
{

#define TRACE_IMX250_PARSE_DT   1   /* DDD - imx250_parse_dt() - trace */

// IMX219 : no parse clocks
//#define PARSE_CLOCKS        0   /* CCC - imx250_parse_dt() - parse clocks */
//#define PARSE_GPIOS         0   /* CCC - imx250_parse_dt() - parse GPIOss */

    struct device *dev = tc_dev->dev;
    struct device_node *node = dev->of_node;
    struct camera_common_pdata *board_priv_pdata;
//    const struct of_device_id *match;
//    struct imx250 *priv = (struct imx250 *)tegracam_get_privdata(tc_dev);
...
//    match = of_match_device(imx250_of_match, dev);
//    if (!match) {
...
/****** vc_mipi_reset = Reset VC MIPI sensor = 11.2020 ******************/
...
/****** vc_mipi_common_trigmode_write = Trigger mode setup = 11.2020 ****/
...
/****** imx250_start_streaming = Start streaming = 11.2020 **************/
static int imx250_start_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX250_START_STREAMING       1   /* DDD - imx250_start_streaming() - trace */
#define IMX250_START_STREAMING_SET_CTRLS   1   /* CCC - imx250_start_streaming() - set controls */
#define IMX250_START_STREAMING_DELAY       0   // 300 /* CCC - imx250_start_streaming() - delay in ms after start, 0=off */
#define VC_EXT_TRIG_SET_EXP                1   /* CCC - imx250_start_streaming() - set exposure in ext. trigger code */
...
#if TRACE_IMX250_START_STREAMING
//    dev_err(dev, "%s():...\n", __func__);
#endif


// Set gain and exposure before streaming start
#if IMX250_START_STREAMING_SET_CTRLS
      imx250_set_gain(tc_dev, priv->digital_gain);
      imx250_set_exposure(tc_dev, priv->exposure_time);
      imx250_set_frame_rate(tc_dev, priv->frame_rate);
//      imx250_set_frame_length(tc_dev, priv->frame_length);
      mdelay(100);
#endif

#if TRACE_IMX250_START_STREAMING
//    dev_info(dev, "%s(): Set frame_length=%d\n", __func__, IMX250_DEFAULT_FRAME_LENGTH);
//    dev_info(dev, "%s(): Set gain=%d\n", __func__, IMX250_DEFAULT_GAIN);
#endif
...
//    mutex_lock(&priv->streaming_lock);
    err = imx250_write_table(priv, imx250_mode_table[IMX250_MODE_START_STREAM]);
    if (err) {
...
#if IMX250_START_STREAMING_DELAY
    dev_err(dev, "%s(): Delay after streaming start: %d ms\n", __func__, IMX250_START_STREAMING_DELAY);
    mdelay(IMX250_START_STREAMING_DELAY);    // ??? delay after start streaming
#endif
...
/****** imx250_stop_streaming = Stop streaming = 11.2020 ****************/
static int imx250_stop_streaming(struct tegracam_device *tc_dev)
{

#define TRACE_IMX250_STOP_STREAMING     1   /* DDD - imx250_stop_streaming() - trace */
//#define IMX250_STOP_STREAMING_ENB_RESET    0    /* CCC - imx250_stop_streaming() - enable VC MIPI reset */
...
#if TRACE_IMX250_STOP_STREAMING
//    dev_err(dev, "%s():\n", __func__);
#endif

    err = imx250_write_table(priv, imx250_mode_table[IMX250_MODE_STOP_STREAM]);
    if (err)
    {
//#if TRACE_IMX250_STOP_STREAMING
      dev_err(dev, "%s(): imx250_write_table() err=%d\n", __func__, err);
...
//#if IMX250_STOP_STREAMING_ENB_RESET  // [[[
...
static int imx250_set_mode(struct tegracam_device *tc_dev);

static struct camera_common_sensor_ops imx250_common_ops = {
    .numfrmfmts = ARRAY_SIZE(imx250_frmfmt),
    .frmfmt_table = imx250_frmfmt,
    .power_on = imx250_power_on,
    .power_off = imx250_power_off,
    .write_reg = imx250_write_reg,
    .read_reg = imx250_read_reg,
    .parse_dt = imx250_parse_dt,
    .power_get = imx250_power_get,
    .power_put = imx250_power_put,
    .set_mode = imx250_set_mode,
    .start_streaming = imx250_start_streaming,
    .stop_streaming = imx250_stop_streaming,
};

/****** imx250_set_mode = Set mode = 11.2020 ****************************/
static int imx250_set_mode(struct tegracam_device *tc_dev)
{

#define TRACE_IMX250_SET_MODE            1   /* DDD - imx250_set_mode() - trace */
#define IMX250_SET_MODE_DUMP_DT_PARAMS   0   /* DDD - imx250_set_mode() - dump DT params */
#define DUMP_SENSOR_MODE                 1   /* DDD - imx250_set_mode() - dump sensor mode */
#define DUMP_IMX250_REGS1                0   /* DDD - imx250_set_mode() - dump sensor regs aft set mode */

//#if !SKIP_IMX250_SET_MODE
    struct imx250 *priv = (struct imx250 *)tegracam_get_privdata(tc_dev);
//#endif
    struct camera_common_data *s_data = tc_dev->s_data;
    struct device *dev = tc_dev->dev;
...
#if IMX250_TRIG_FIX || IMX250_SET_MODE_DUMP_DT_PARAMS
    struct sensor_mode_properties *mode =  &s_data->sensor_props.sensor_modes[s_data->mode_prop_idx];
#endif
...
#if IMX250_TRIG_FIX   // trig mode fix
        tc_dev->s_data->numfmts = ARRAY_SIZE(imx250_trig_frmfmt);  // tc_dev->sensor_ops->numfrmfmts;
        tc_dev->s_data->frmfmt  = imx250_trig_frmfmt;             // tc_dev->sensor_ops->frmfmt_table;

        imx250_common_ops.numfrmfmts = ARRAY_SIZE(imx250_trig_frmfmt);
        imx250_common_ops.frmfmt_table = imx250_trig_frmfmt;

        mode->image_properties.height = IMX250_DY-2;
        s_data->fmt_height = IMX250_DY-2;
#endif
      }
#if IMX250_TRIG_FIX   // trig mode fix
      else    // free-run mode
      {
        tc_dev->s_data->numfmts = ARRAY_SIZE(imx250_frmfmt);  // tc_dev->sensor_ops->numfrmfmts;
        tc_dev->s_data->frmfmt  = imx250_frmfmt;              // tc_dev->sensor_ops->frmfmt_table;

        imx250_common_ops.numfrmfmts = ARRAY_SIZE(imx250_frmfmt);
        imx250_common_ops.frmfmt_table = imx250_frmfmt;

        mode->image_properties.height = IMX250_DY;
        s_data->fmt_height = IMX250_DY;
      }
#endif
      if(sensor_mode_id == 0)
...
//    if(s_data->fmt_width >= 1920 && s_data->fmt_height >= 1080)
    {
      priv->cam_mode = IMX250_MODE_2432X2048;
    }
//    else
//    {
...
//    if(priv->cam_mode != IMX250_MODE_2432X2048)
    {
      err = imx250_write_table(priv, imx250_mode_table[priv->cam_mode]);
      if (err)
      {
        dev_err(dev, "%s(): imx250_write_table() error=%d\n", __func__, err);
//        goto exit;
      }
      else
      {
#if TRACE_IMX250_SET_MODE
//        dev_err(dev, "%s(): imx250_write_table() OK, cam_mode=%d\n", __func__, priv->cam_mode);
#endif
      }
    }

#if IMX250_SET_MODE_DUMP_DT_PARAMS
{
//    struct sensor_mode_properties *mode =  &s_data->sensor_props.sensor_modes[s_data->mode_prop_idx];
...
/****** imx250_video_probe = Video probe = 11.2020 **********************/
static int imx250_video_probe(struct i2c_client *client)
{
...
// new video probe:
///****** imx_video_probe = IMX video probe = 11.2020 *********************/
...
/****** imx250_probe_vc_rom = Probe VC ROM = 11.2020 ********************/
static struct i2c_client * imx250_probe_vc_rom(struct i2c_adapter *adapter, u8 addr)
{
...
/****** imx250_board_setup = Board setup = 11.2020 **********************/
static int imx250_board_setup(struct imx250 *priv)
{

#define TRACE_IMX250_BOARD_SETUP   1   /* DDD - imx250_board_setup() - trace */
#define DUMP_CTL_REG_DATA          0   /* DDD - imx250_board_setup() - dump module control registers 0x100-0x108 (I2C addr=0x10) */
#define DUMP_HWD_DESC_ROM_DATA     0   /* DDD - imx250_board_setup() - dump Hardware Desriptor ROM data (I2C addr=0x10) */
#define DUMP_IMX250_REGS           0   /* DDD - imx250_board_setup() - dump IMX250 regs */
#define DUMP_V4L_PARAMS            1   /* DDD - imx250_board_setup() - dump V4L params */
#define DUMP_ROM_TABLE_REGS        0   /* DDD - imx250_board_setup() - dump ROM table regs */

//#define MOD_ID                0x0250   /* CCC - imx250_board_setup() - module id., 0=off: don't check */
//#define ERR_MOD_ID                 0   /* DDD - imx250_board_setup() - invalid module id test */
...
//    priv->rom = i2c_new_dummy(adapter,fpga_addr);
    priv->rom = imx250_probe_vc_rom(adapter,priv->fpga_addr);
...
//................ Check model:
        ...
        // IMX250
        if(priv->rom_table.mod_id == 0x0250)
        {
          if(color)
          {
            priv->model = IMX250_MODEL_COLOR;
          }
          else
          {
            priv->model = IMX250_MODEL_MONOCHROME;
          }
        }
...
//[    2.195918] imx250 7-001a: imx250_board_setup(): Module controller registers (0x10):
//[    2.208811] imx250 7-001a: 0x100-0x103: 00 80 01 08
//[    2.213749] imx250 7-001a: 0x104-0x108: 10 1a 00 00 00
//[    2.218978] imx250 7-001a: 0x109-0x10C: 10 27 00 00
//[    2.223926] imx250 7-001a: 0x10D-0x110: 3e ec 07 00
    ...
    ret = imx250_video_probe(client);
    if (ret < 0)
    {
      dev_err(dev, "%s(): imx250_video_probe() error=%d\n", __func__, ret);
      err = -EIO;
      goto done;
    }
...
/****** imx250_open = Open device = 11.2020 *****************************/
static int imx250_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
...
static const struct v4l2_subdev_internal_ops imx250_subdev_internal_ops = {
    .open = imx250_open,
};

static const struct of_device_id imx250_of_match[] = {
    { .compatible = "nvidia,imx250", },
    { },
};
MODULE_DEVICE_TABLE(of, imx250_of_match);

/****** imx250_probe = Probe = 11.2020 **********************************/
static int imx250_probe(struct i2c_client *client,
...
//    dev_info(dev, "%s(): Probing v4l2 sensor at addr 0x%0x\n", __func__, client->addr); // , __DATE__, __TIME__);
    ...
    priv = devm_kzalloc(dev,
                sizeof(struct imx250), GFP_KERNEL);
    if (!priv)
        return -ENOMEM;
    ...
    tc_dev->dev = dev;
    strncpy(tc_dev->name, "imx250", sizeof(tc_dev->name));
    tc_dev->dev_regmap_config = &imx250_regmap_config;
    tc_dev->sensor_ops = &imx250_common_ops;
    tc_dev->v4l2sd_internal_ops = &imx250_subdev_internal_ops;
    tc_dev->tcctrl_ops = &imx250_ctrl_ops;
    ...
    err = imx250_board_setup(priv);
    if (err) {
        dev_err(dev, "%s: imx250_board_setup() error=%d\n", __func__, err);
        return err;
    }
    dev_info(dev, "%s(): imx250_board_setup() OK\n", __func__);
    ...
    priv->digital_gain  = IMX250_DIGITAL_GAIN_DEFAULT;
    priv->exposure_time = IMX250_DIGITAL_EXPOSURE_DEFAULT;
    priv->frame_rate    = IMX250_FRAME_RATE_DEFAULT;
//    priv->frame_length = IMX250_DEFAULT_FRAME_LENGTH;
    if(ext_trig_mode >= 0)
...
//    priv->sen_clk = 13000000;       // clock-frequency: imx250=74Mhz
...

{
    ...
    mode->control_properties.default_gain         = IMX250_DIGITAL_GAIN_DEFAULT;
    mode->control_properties.default_exp_time.val = IMX250_DIGITAL_EXPOSURE_DEFAULT;
}


//    set_sensor_model(imx250_sensor_model);
    set_sensor_model("imx250");

//    dev_err(dev, "%s(): Detected imx250 sensor - %s/%s\n", __func__, __DATE__, __TIME__);

    switch(priv->model)
    {
      case IMX250_MODEL_MONOCHROME:
        priv->cam_mode = IMX250_MODE_2432X2048;
        dev_err(dev, "%s(): Detected imx250 sensor\n", __func__); // , __DATE__, __TIME__);
        break;

      case IMX250_MODEL_COLOR:
        priv->cam_mode = IMX250_MODE_2432X2048;
        dev_err(dev, "%s(): Detected imx250c sensor\n", __func__); // , __DATE__, __TIME__);
        break;
...
/****** imx250_remove = Remove = 11.2020 ********************************/
static int imx250_remove(struct i2c_client *client)
{
    struct camera_common_data *s_data = to_camera_common_data(&client->dev);
    struct imx250 *priv = (struct imx250 *)s_data->priv;
...
static const struct i2c_device_id imx250_id[] = {
    { "imx250", 0 },
    { }
};

MODULE_DEVICE_TABLE(i2c, imx250_id);



/****** i2c_driver = I2C driver = o9.2020 *******************************/
static struct i2c_driver imx250_i2c_driver = {
    .driver = {
        .name = "imx250",
        .owner = THIS_MODULE,
        .of_match_table = of_match_ptr(imx250_of_match),
    },
    .probe = imx250_probe,
    .remove = imx250_remove,
    .id_table = imx250_id,
};
module_i2c_driver(imx250_i2c_driver);

MODULE_VERSION("1.04");
MODULE_DESCRIPTION("Media Controller driver for IMX250");
