...
static void vc_core_print_modes(struct device *dev, struct vc_desc *desc)
{
	...
	for (index = 0; index<desc->num_modes; index++) {
		...
		default: sprintf(format, "0x%02x     ", mode->type); break;
		}
...
static int vc_mod_write_flash_mode(struct i2c_client *client, int mode)
{
	struct device *dev = &client->dev;
	int ret;

	vc_dbg(dev, "%s(): Write flash mode: %s\n", __FUNCTION__, mode ? "ON" : "OFF");
...
static int vc_sen_write_exposure(struct vc_ctrl *ctrl, __u32 exposure)
{
	...
	vc_dbg(dev, "%s(): Write sensor exposure: 0x%08x (%u)\n", __FUNCTION__, exposure, exposure);
...
int vc_sen_stop_stream(struct vc_cam *cam)
{
	...
	ret |= vc_mod_write_flash_mode(client_mod, REG_FLASH_DISABLE);
...
// static void vc_calculate_exposure_vmax_V2(struct vc_cam *cam, int value, int *exposure, int *vmax)
// {
// 	struct vc_ctrl *ctrl = &cam->ctrl;
// 	struct device *dev = &ctrl->client_sen->dev;
